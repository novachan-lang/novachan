...
/* read_bytes(n) — read exactly n bytes from stdin (binary mode).
   Returns a fat string.  For LSP: Content-Length header says how many
   bytes to read, and we must not translate CR/LF on Windows.           */
int64_t nova_rt_read_bytes(int64_t count_val) {
    if (count_val <= 0) {
        char* empty = nova_fat_str_create("", 0);
        return empty ? (int64_t)(uintptr_t)empty : 0;
    }
    /* Clamp to 512 MB — same limit as read_file */
    if (count_val > (int64_t)512 * 1024 * 1024) {
        nova_set_error("read_bytes: count exceeds 512MB limit");
        char* empty = nova_fat_str_create("", 0);
        return empty ? (int64_t)(uintptr_t)empty : 0;
    }
#ifdef _WIN32
    ...
    if (!tmp) {
        nova_set_error("read_bytes: out of memory");
        char* empty = nova_fat_str_create("", 0);
        return empty ? (int64_t)(uintptr_t)empty : 0;
    }
    size_t total_read = 0;
    while (total_read < count) {
        size_t got = fread(tmp + total_read, 1, count - total_read, stdin);
        if (got == 0) {
            /* EOF or read error before we got all requested bytes */
            if (feof(stdin)) {
                nova_set_error("read_bytes: unexpected EOF");
            } else {
                nova_set_error("read_bytes: fread failed");
            }
            break;
        }
        total_read += got;
    }
    char* result = nova_fat_str_create(tmp, total_read);
    free(tmp);
    if (!result) return 0;
    return (int64_t)(uintptr_t)result;
}

/* write_raw(s) — write string bytes to stdout without trailing newline.
   Binary mode on Windows to avoid CR/LF translation.  Flushes stdout.  */
int64_t nova_rt_write_raw(int64_t str_val) {
    if (str_val == 0) return 0;
    const char* s = (const char*)(uintptr_t)str_val;
#ifdef _WIN32
    _setmode(_fileno(stdout), _O_BINARY);
#endif
    /* Determine length: use fat string length if available, else strlen */
    size_t len;
    NovaMemTag tag = nova_mem_find_tag((void*)s);
    if (tag == NOVA_MEM_FAT_STR) {
        len = (size_t)NOVA_FAT_LEN(s);
    } else {
        len = strlen(s);
    }
    if (len > 0) {
        size_t written = fwrite(s, 1, len, stdout);
        (void)written; /* nothing useful to do on partial write to stdout */
    }
    fflush(stdout);
    return 0;
}
...
int64_t nova_rt_any_to_str(int64_t val) {
    ...
    NovaMemTag tag = nova_mem_find_tag(ptr);
    switch (tag) {
        case NOVA_MEM_RAW:      nova_rc_inc(val); return val;
        case NOVA_MEM_FAT_STR:  nova_rc_inc(val); return val;
        case NOVA_MEM_LIST:     return nova_rt_list_to_str(val);
        case NOVA_MEM_DICT:     return nova_rt_json_stringify(val);
        default:
            if ((uint64_t)val > 0x10000 && nova_is_readable_str(ptr)) {
                unsigned char c = *(unsigned char*)ptr;
                if (c == 0 || (c >= 0x20 && c < 0x7F)) { nova_rc_inc(val); return val; }
            }
...
int64_t nova_rt_type_of(int64_t val) {
    const char* name;
    size_t len;
    if (val == 0) { name = "int"; len = 3; }
    else {
        void* ptr = (void*)(uintptr_t)val;
        NovaMemTag tag = nova_mem_find_tag(ptr);
        switch (tag) {
            case NOVA_MEM_RAW:     name = "string"; len = 6; break;
            case NOVA_MEM_FAT_STR: name = "string"; len = 6; break;
            case NOVA_MEM_LIST:    name = "list";   len = 4; break;
            case NOVA_MEM_DICT:    name = "dict";   len = 4; break;
            case NOVA_MEM_CHANNEL: name = "channel"; len = 7; break;
            default:
                if ((uint64_t)val > 0x10000 && nova_is_readable_str(ptr)) {
                    unsigned char c = *(unsigned char*)ptr;
                    if (c == 0 || (c >= 0x20 && c < 0x7F)) { name = "string"; len = 6; break; }
                }
                name = "int"; len = 3;
                break;
        }
    }
    char* r = nova_fat_str_create(name, len);
...
int64_t nova_rt_range(int64_t n) {
    ...
    for (int64_t i = 0; i < n; i++) {
        nova_rt_list_append(list, i);
    }
    return list;
}

int64_t nova_rt_range_from_to(int64_t start, int64_t end) {
    int64_t list = nova_rt_list_create();
    for (int64_t i = start; i < end; i++) {
        nova_rt_list_append(list, i);
    }
...
/* ── Collection utilities: enumerate, zip, sum, any, all, reversed ────────── */

int64_t nova_rt_enumerate(int64_t list_handle) {
    NovaList* l = (NovaList*)(uintptr_t)list_handle;
...
int64_t nova_rt_zip(int64_t a_handle, int64_t b_handle) {
    NovaList* a = (NovaList*)(uintptr_t)a_handle;
    NovaList* b = (NovaList*)(uintptr_t)b_handle;
    int64_t min_sz = a->size < b->size ? a->size : b->size;
    int64_t result = nova_rt_list_create();
    for (int64_t i = 0; i < min_sz; i++) {
        int64_t pair = nova_rt_list_create();
...
int64_t nova_rt_list_sum(int64_t list_handle) {
    NovaList* l = (NovaList*)(uintptr_t)list_handle;
    int64_t sum = 0;
...
int64_t nova_rt_list_any(int64_t list_handle) {
    NovaList* l = (NovaList*)(uintptr_t)list_handle;
    for (int64_t i = 0; i < l->size; i++) {
...
int64_t nova_rt_list_all(int64_t list_handle) {
    NovaList* l = (NovaList*)(uintptr_t)list_handle;
    for (int64_t i = 0; i < l->size; i++) {
...
int64_t nova_rt_list_min(int64_t list_handle) {
    NovaList* l = (NovaList*)(uintptr_t)list_handle;
...
int64_t nova_rt_list_max(int64_t list_handle) {
    NovaList* l = (NovaList*)(uintptr_t)list_handle;
    if (l->size == 0) return 0;
    int64_t m = l->data[0];
    for (int64_t i = 1; i < l->size; i++) {
...
/* ── Set (hash-based, integer/string keys) ───────────────────────────────── */
...
int64_t nova_rt_set_create(void) {
    NovaSet* s = (NovaSet*)nova_heap_alloc(sizeof(NovaSet), NOVA_MEM_RAW);
    if (!s) return 0;
    s->data = (int64_t*)malloc(16 * sizeof(int64_t));
    s->size = 0;
    s->cap  = 16;
    return (int64_t)(uintptr_t)s;
}

static int set_contains(NovaSet* s, int64_t val) {
...
int64_t nova_rt_set_add(int64_t handle, int64_t val) {
    NovaSet* s = (NovaSet*)(uintptr_t)handle;
    if (set_contains(s, val)) return 0;
    if (s->size >= s->cap) {
        s->cap *= 2;
        s->data = (int64_t*)realloc(s->data, (size_t)s->cap * sizeof(int64_t));
    }
    s->data[s->size++] = val;
    return 1;
}

int64_t nova_rt_set_has(int64_t handle, int64_t val) {
    NovaSet* s = (NovaSet*)(uintptr_t)handle;
    return set_contains(s, val) ? 1 : 0;
}

int64_t nova_rt_set_remove(int64_t handle, int64_t val) {
    NovaSet* s = (NovaSet*)(uintptr_t)handle;
    for (int64_t i = 0; i < s->size; i++) {
        if (s->data[i] == val) {
...
int64_t nova_rt_set_to_list(int64_t handle) {
    ...
    int64_t list = nova_rt_list_create();
    for (int64_t i = 0; i < s->size; i++) {
        nova_rt_list_append(list, s->data[i]);
    }
    return list;
}
...
int64_t nova_rt_datetime_now(void) {
    ...
    size_t len = strlen(buf);
    char* r = (char*)nova_heap_alloc(len + 1, NOVA_MEM_RAW);
    if (!r) return 0;
    memcpy(r, buf, len + 1);
    return (int64_t)(uintptr_t)r;
}

int64_t nova_rt_datetime_format(int64_t ms_timestamp, int64_t fmt_ptr) {
    time_t t = (time_t)(ms_timestamp / 1000);
...
int64_t nova_rt_datetime_year(int64_t ms_timestamp) {
    time_t t = (time_t)(ms_timestamp / 1000);
    struct tm* tm_info;
#ifdef _WIN32
    struct tm tm_buf;
...
int64_t nova_rt_datetime_month(int64_t ms_timestamp) {
    time_t t = (time_t)(ms_timestamp / 1000);
    struct tm* tm_info;
#ifdef _WIN32
    struct tm tm_buf;
...
int64_t nova_rt_datetime_day(int64_t ms_timestamp) {
    time_t t = (time_t)(ms_timestamp / 1000);
    struct tm* tm_info;
#ifdef _WIN32
    struct tm tm_buf;
...
int64_t nova_rt_datetime_hour(int64_t ms_timestamp) {
    time_t t = (time_t)(ms_timestamp / 1000);
    struct tm* tm_info;
#ifdef _WIN32
    struct tm tm_buf;
...
int64_t nova_rt_datetime_minute(int64_t ms_timestamp) {
    time_t t = (time_t)(ms_timestamp / 1000);
    struct tm* tm_info;
#ifdef _WIN32
    struct tm tm_buf;
...
int64_t nova_rt_datetime_second(int64_t ms_timestamp) {
    time_t t = (time_t)(ms_timestamp / 1000);
    struct tm* tm_info;
#ifdef _WIN32
    struct tm tm_buf;
...
/* ═══════════════════════════════════════════════════════════════════════════
 *  DISTRIBUTED COMPUTING — Serialization, Remote Channels, Multi-Node Spawn
 * ═══════════════════════════════════════════════════════════════════════════ */
...
static void sbuf_init(NovaSBuf* b) {
    b->cap = 256;
    b->data = (uint8_t*)malloc(b->cap);
    b->len = 0;
}

static void sbuf_push(NovaSBuf* b, const void* src, size_t n) {
    if (!b->data) return;
    while (b->len + n > b->cap) {
        b->cap *= 2;
        uint8_t* nd = (uint8_t*)realloc(b->data, b->cap);
        if (!nd) { free(b->data); b->data = NULL; return; }
        b->data = nd;
    }
...
static void sbuf_push_u8(NovaSBuf* b, uint8_t v) { sbuf_push(b, &v, 1); }
static void sbuf_push_u32(NovaSBuf* b, uint32_t v) { sbuf_push(b, &v, 4); }
static void sbuf_push_i64(NovaSBuf* b, int64_t v) { sbuf_push(b, &v, 8); }

/* ── Serialization type tags ───────────────────────────────────────────── */

#define NOVA_SER_NIL    0x00
#define NOVA_SER_INT    0x01
#define NOVA_SER_STRING 0x03
#define NOVA_SER_LIST   0x04
#define NOVA_SER_DICT   0x05

static void serialize_value(NovaSBuf* buf, int64_t val);
static int64_t deserialize_value(const uint8_t** ptr, const uint8_t* end);

static void serialize_value(NovaSBuf* buf, int64_t val) {
    if (val == 0) {
        sbuf_push_u8(buf, NOVA_SER_INT);
        sbuf_push_i64(buf, 0);
        return;
    }
    void* ptr = (void*)(uintptr_t)val;
    ...
    switch (tag) {
        case NOVA_MEM_RAW:
        case NOVA_MEM_FAT_STR: {
            sbuf_push_u8(buf, NOVA_SER_STRING);
            const char* s = (const char*)ptr;
            uint32_t slen = (uint32_t)strlen(s);
            sbuf_push_u32(buf, slen);
            sbuf_push(buf, s, slen);
            break;
        }
        case NOVA_MEM_LIST: {
            sbuf_push_u8(buf, NOVA_SER_LIST);
            NovaList* list = (NovaList*)ptr;
            uint32_t count = (uint32_t)list->size;
            sbuf_push_u32(buf, count);
            for (uint32_t i = 0; i < count; i++) {
                serialize_value(buf, list->data[i]);
            }
            break;
        }
        case NOVA_MEM_DICT: {
            sbuf_push_u8(buf, NOVA_SER_DICT);
            NovaDict* d = (NovaDict*)ptr;
            uint32_t count = (uint32_t)d->size;
            sbuf_push_u32(buf, count);
            for (int64_t i = 0; i < d->size; i++) {
                serialize_value(buf, d->keys[i]);
                serialize_value(buf, d->vals[i]);
            }
            break;
        }
        default: {
...
static int64_t deserialize_value(const uint8_t** ptr, const uint8_t* end) {
    ...
    switch (tag) {
        ...
        case NOVA_SER_STRING: {
            if (*ptr + 4 > end) return 0;
            uint32_t slen;
            memcpy(&slen, *ptr, 4);
            *ptr += 4;
            if (*ptr + slen > end) return 0;
            char* s = nova_fat_str_create((const char*)*ptr, slen);
            *ptr += slen;
            return s ? (int64_t)(uintptr_t)s : 0;
        }
        case NOVA_SER_LIST: {
            if (*ptr + 4 > end) return 0;
            uint32_t count;
            ...
            *ptr += 4;
            int64_t list = nova_rt_list_create();
            for (uint32_t i = 0; i < count; i++) {
                nova_rt_list_append(list, deserialize_value(ptr, end));
            }
            return list;
        }
        case NOVA_SER_DICT: {
            ...
            *ptr += 4;
            int64_t dict = nova_rt_dict_create();
            for (uint32_t i = 0; i < count; i++) {
                int64_t key = deserialize_value(ptr, end);
                int64_t val = deserialize_value(ptr, end);
                nova_rt_dict_set(dict, key, val);
            }
            return dict;
        }
...
int64_t nova_rt_serialize(int64_t val) {
    NovaSBuf buf;
    sbuf_init(&buf);
    serialize_value(&buf, val);
    if (!buf.data) return nova_rt_bytes_create(0);
    int64_t result = nova_rt_bytes_create((int64_t)buf.len);
    NovaBytes* b = (NovaBytes*)(uintptr_t)result;
    if (b && b->data) memcpy(b->data, buf.data, buf.len);
    free(buf.data);
    return result;
}

int64_t nova_rt_deserialize(int64_t bytes_handle) {
...
/* ── Framed TCP — length-prefixed binary messages ──────────────────────── */

static int tcp_send_all(NOVA_SOCKET sock, const void* data, size_t len) {
...
static int tcp_recv_all(NOVA_SOCKET sock, void* data, size_t len) {
    char* p = (char*)data;
    size_t got = 0;
    while (got < len) {
...
static int nova_send_frame(NOVA_SOCKET sock, uint8_t msg_type,
                           const uint8_t* payload, uint32_t payload_len) {
    uint32_t total = 1 + payload_len;
    if (tcp_send_all(sock, &total, 4) != 0) return -1;
    if (tcp_send_all(sock, &msg_type, 1) != 0) return -1;
    if (payload_len > 0 && tcp_send_all(sock, payload, payload_len) != 0) return -1;
    return 0;
}

static int nova_recv_frame(NOVA_SOCKET sock, uint8_t* out_type,
                           uint8_t** out_payload, uint32_t* out_len) {
    uint32_t total;
    if (tcp_recv_all(sock, &total, 4) != 0) return -1;
    if (total < 1 || total > 64 * 1024 * 1024) return -1;
    uint8_t msg_type;
    if (tcp_recv_all(sock, &msg_type, 1) != 0) return -1;
    *out_type = msg_type;
    uint32_t plen = total - 1;
    *out_len = plen;
    if (plen > 0) {
        *out_payload = (uint8_t*)malloc(plen);
        if (!*out_payload) return -1;
        if (tcp_recv_all(sock, *out_payload, plen) != 0) {
            free(*out_payload);
            *out_payload = NULL;
            return -1;
        }
...
/* ── Node protocol message types ───────────────────────────────────────── */

#define NOVA_MSG_SPAWN       0x01
#define NOVA_MSG_RESULT      0x02
#define NOVA_MSG_SEND        0x03
#define NOVA_MSG_RECV        0x04
#define NOVA_MSG_RECV_OK     0x05
#define NOVA_MSG_CLOSE       0x06
#define NOVA_MSG_HEARTBEAT   0x07
#define NOVA_MSG_ERROR       0x08
#define NOVA_MSG_HANDSHAKE   0x10
#define NOVA_MSG_HANDSHAKE_OK 0x11

/* ── Remote channel: transparent send/recv over TCP ────────────────────── */
...
int64_t nova_rt_remote_channel_create(int64_t sock_val) {
    NovaRemoteChannel* rc = (NovaRemoteChannel*)malloc(sizeof(NovaRemoteChannel));
    if (!rc) return 0;
    rc->sock = (NOVA_SOCKET)sock_val;
    rc->is_server = 0;
#ifdef _WIN32
    InitializeCriticalSection(&rc->lock);
#else
    pthread_mutex_init(&rc->lock, NULL);
#endif
    return (int64_t)(uintptr_t)rc;
}

int64_t nova_rt_remote_send(int64_t rch_handle, int64_t val) {
    NovaRemoteChannel* rc = (NovaRemoteChannel*)(uintptr_t)rch_handle;
    if (!rc) { nova_set_error("remote_send: null channel"); return -1; }

    NovaSBuf buf;
    sbuf_init(&buf);
    serialize_value(&buf, val);
    if (!buf.data) { nova_set_error("remote_send: serialize failed"); return -1; }

#ifdef _WIN32
    EnterCriticalSection(&rc->lock);
#else
    pthread_mutex_lock(&rc->lock);
#endif
    int err = nova_send_frame(rc->sock, NOVA_MSG_SEND, buf.data, (uint32_t)buf.len);
#ifdef _WIN32
    LeaveCriticalSection(&rc->lock);
#else
    pthread_mutex_unlock(&rc->lock);
#endif
    free(buf.data);
    if (err != 0) { nova_set_error("remote_send: network error"); return -1; }
    return 0;
}

int64_t nova_rt_remote_recv(int64_t rch_handle) {
    NovaRemoteChannel* rc = (NovaRemoteChannel*)(uintptr_t)rch_handle;
    if (!rc) { nova_set_error("remote_recv: null channel"); return 0; }
    ...
    pthread_mutex_lock(&rc->lock);
#endif
    int err = nova_recv_frame(rc->sock, &msg_type, &payload, &plen);
#ifdef _WIN32
    LeaveCriticalSection(&rc->lock);
#else
    pthread_mutex_unlock(&rc->lock);
    ...
    if (err != 0) { nova_set_error("remote_recv: network error"); return 0; }
    if (msg_type == NOVA_MSG_ERROR) {
        if (payload && plen > 0) {
            char* emsg = (char*)malloc(plen + 1);
            if (emsg) { memcpy(emsg, payload, plen); emsg[plen] = 0; nova_set_error(emsg); free(emsg); }
        }
        free(payload);
        return 0;
    }
    if (msg_type != NOVA_MSG_SEND && msg_type != NOVA_MSG_RESULT) {
        free(payload);
        return 0;
    }
    if (!payload || plen == 0) { free(payload); return 0; }
    const uint8_t* p = payload;
    const uint8_t* end = payload + plen;
    int64_t result = deserialize_value(&p, end);
    free(payload);
    return result;
}

void nova_rt_remote_close(int64_t rch_handle) {
    NovaRemoteChannel* rc = (NovaRemoteChannel*)(uintptr_t)rch_handle;
    if (!rc) return;
    nova_send_frame(rc->sock, NOVA_MSG_CLOSE, NULL, 0);
    NOVA_CLOSE_SOCKET(rc->sock);
#ifdef _WIN32
    DeleteCriticalSection(&rc->lock);
#else
...
/* ── Node: listen for incoming connections and spawn work ──────────────── */

typedef struct {
    NOVA_SOCKET server_sock;
    int         running;
    int64_t     port;
} NovaNode;

int64_t nova_rt_node_listen(int64_t port_val) {
    int64_t sock = nova_rt_tcp_listen(port_val);
    if (sock < 0) return -1;
    NovaNode* node = (NovaNode*)malloc(sizeof(NovaNode));
    if (!node) { NOVA_CLOSE_SOCKET((NOVA_SOCKET)sock); return -1; }
    node->server_sock = (NOVA_SOCKET)sock;
    node->running = 1;
    node->port = port_val;
    return (int64_t)(uintptr_t)node;
}

int64_t nova_rt_node_accept(int64_t node_handle) {
    NovaNode* node = (NovaNode*)(uintptr_t)node_handle;
    if (!node) { nova_set_error("node_accept: null node"); return -1; }
    int64_t client = nova_rt_tcp_accept((int64_t)node->server_sock);
    if (client < 0) return -1;
    return nova_rt_remote_channel_create(client);
}

void nova_rt_node_stop(int64_t node_handle) {
    NovaNode* node = (NovaNode*)(uintptr_t)node_handle;
    if (!node) return;
    node->running = 0;
    NOVA_CLOSE_SOCKET(node->server_sock);
    free(node);
}

int64_t nova_rt_node_connect(int64_t host_port_str) {
    const char* hp = (const char*)(uintptr_t)host_port_str;
    if (!hp) { nova_set_error("node_connect: null address"); return 0; }

    char host[256];
    int port = 0;
    const char* colon = strrchr(hp, ':');
    if (!colon) { nova_set_error("node_connect: address must be host:port"); return 0; }
    size_t hlen = (size_t)(colon - hp);
    if (hlen >= sizeof(host)) hlen = sizeof(host) - 1;
    memcpy(host, hp, hlen);
    host[hlen] = 0;
    port = atoi(colon + 1);
    if (port <= 0 || port > 65535) { nova_set_error("node_connect: invalid port"); return 0; }

    char* host_str = nova_fat_str_create(host, hlen);
    int64_t sock = nova_rt_tcp_connect((int64_t)(uintptr_t)host_str, (int64_t)port);
    if (sock < 0) return 0;
    return nova_rt_remote_channel_create(sock);
}

/* ── Remote spawn: send function name + args, get result channel ───────── */

int64_t nova_rt_remote_spawn(int64_t rch_handle, int64_t fn_name_val, int64_t args_val) {
    NovaRemoteChannel* rc = (NovaRemoteChannel*)(uintptr_t)rch_handle;
    if (!rc) { nova_set_error("remote_spawn: null channel"); return -1; }

    const char* fn = (const char*)(uintptr_t)fn_name_val;
    if (!fn) { nova_set_error("remote_spawn: null function name"); return -1; }

    NovaSBuf buf;
    sbuf_init(&buf);

    uint16_t fn_len = (uint16_t)strlen(fn);
    sbuf_push(&buf, &fn_len, 2);
    sbuf_push(&buf, fn, fn_len);

    NovaList* args = NULL;
    uint16_t arg_count = 0;
    void* aptr = (void*)(uintptr_t)args_val;
    NovaMemTag atag = nova_mem_find_tag(aptr);
    if (atag == NOVA_MEM_LIST) {
        args = (NovaList*)aptr;
        arg_count = (uint16_t)args->size;
    }
    sbuf_push(&buf, &arg_count, 2);
    for (uint16_t i = 0; i < arg_count; i++) {
        serialize_value(&buf, args->data[i]);
    }

    if (!buf.data) { nova_set_error("remote_spawn: serialize failed"); return -1; }
    ...
    pthread_mutex_lock(&rc->lock);
#endif
    int err = nova_send_frame(rc->sock, NOVA_MSG_SPAWN, buf.data, (uint32_t)buf.len);
#ifdef _WIN32
    LeaveCriticalSection(&rc->lock);
#else
    pthread_mutex_unlock(&rc->lock);
#endif
    free(buf.data);
    if (err != 0) { nova_set_error("remote_spawn: network error"); return -1; }
    return 0;
}

/* ── Node worker: receive and parse SPAWN messages ─────────────────────── */

int64_t nova_rt_node_recv_spawn(int64_t rch_handle) {
    NovaRemoteChannel* rc = (NovaRemoteChannel*)(uintptr_t)rch_handle;
    if (!rc) return 0;
    ...
    uint8_t* payload = NULL;
    uint32_t plen = 0;
    if (nova_recv_frame(rc->sock, &msg_type, &payload, &plen) != 0) return 0;
    if (msg_type != NOVA_MSG_SPAWN) { free(payload); return 0; }
    if (!payload || plen < 4) { free(payload); return 0; }

    const uint8_t* p = payload;
    const uint8_t* end = payload + plen;

    uint16_t fn_len;
    memcpy(&fn_len, p, 2); p += 2;
    if (p + fn_len > end) { free(payload); return 0; }
    char* fn_name = nova_fat_str_create((const char*)p, fn_len);
    p += fn_len;

    uint16_t arg_count;
    memcpy(&arg_count, p, 2); p += 2;

    int64_t args_list = nova_rt_list_create();
    for (uint16_t i = 0; i < arg_count; i++) {
        int64_t val = deserialize_value(&p, end);
        nova_rt_list_append(args_list, val);
    }
    free(payload);

    int64_t result = nova_rt_dict_create();
    nova_rt_dict_set(result, (int64_t)(uintptr_t)"fn", (int64_t)(uintptr_t)fn_name);
    nova_rt_dict_set(result, (int64_t)(uintptr_t)"args", args_list);
    return result;
}

/* ── Convenience: serialize a value to a printable hex string (debug) ──── */

int64_t nova_rt_serialize_to_hex(int64_t val) {
    NovaSBuf buf;
    sbuf_init(&buf);
    serialize_value(&buf, val);
    if (!buf.data) return (int64_t)(uintptr_t)"";

    size_t hex_len = buf.len * 2;
    char* hex = (char*)nova_heap_alloc(hex_len + 1, NOVA_MEM_RAW);
    if (!hex) { free(buf.data); return (int64_t)(uintptr_t)""; }
    static const char hx[] = "0123456789abcdef";
    for (size_t i = 0; i < buf.len; i++) {
        hex[i*2]   = hx[buf.data[i] >> 4];
        hex[i*2+1] = hx[buf.data[i] & 0xF];
    }
    hex[hex_len] = 0;
    free(buf.data);
    return (int64_t)(uintptr_t)hex;
}
