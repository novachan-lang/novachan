...
/* ── Missing runtime functions for self-hosted bootstrap ────────────────── */

static int nova_stored_argc = 0;
static char** nova_stored_argv = NULL;

void nova_rt_init_args(int64_t argc, int64_t argv) {
    nova_stored_argc = (int)argc;
    nova_stored_argv = (char**)(uintptr_t)argv;
}

int64_t nova_rt_args(void) {
    int64_t list = nova_rt_list_create();
    for (int i = 0; i < nova_stored_argc; i++) {
        const char* arg = nova_stored_argv[i];
...
int64_t nova_rt_print_any(int64_t val) {
    ...
    const char* cs = (const char*)(uintptr_t)s;
    if (cs) printf("%s\n", cs);
    return 0;
}

int64_t nova_rt_float_bits(int64_t val) {
    return val;
}

int64_t nova_rt_system(int64_t cmd) {
    const char* c = (const char*)(uintptr_t)cmd;
    int ret = system(c);
    return (int64_t)ret;
}
