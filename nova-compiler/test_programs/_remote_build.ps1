...
$l1 = Invoke-Timed -FilePath $ClangPath -Arguments "-O2 -o `"$dir\gen4_rm.exe`" `"$dir\nova_compiler.ll`" `"$dir\output\nova_runtime.c`" $NovaLinkFlags -D_CRT_SECURE_NO_WARNINGS -w" -TimeoutMs 150000
if (!(Test-Path "$dir\gen4_rm.exe")) { Write-Host "LINK-FAIL gen4 $($l1.StdErr)"; exit 1 }
Write-Host "  gen4_rm.exe built"
Write-Host "[2] gen4 -> compile remote_test.nova"
$c2 = Invoke-Timed -FilePath "$dir\gen4_rm.exe" -Arguments "remote_test.nova" -TimeoutMs 60000
if ($c2.ExitCode -ne 0) { Write-Host "COMPILE-FAIL set_comp exit=$($c2.ExitCode)"; Write-Host $c2.StdErr; exit 1 }
$l2 = Invoke-Timed -FilePath $ClangPath -Arguments "-O2 -o `"$dir\remote_test.exe`" `"$dir\remote_test.ll`" `"$dir\output\nova_runtime.c`" $NovaLinkFlags -D_CRT_SECURE_NO_WARNINGS -w" -TimeoutMs 120000
if (!(Test-Path "$dir\remote_test.exe")) { Write-Host "LINK-FAIL set_comp $($l2.StdErr)"; exit 1 }
Write-Host "[3] run remote_test"
$r = Invoke-Timed -FilePath "$dir\remote_test.exe" -TimeoutMs 15000
