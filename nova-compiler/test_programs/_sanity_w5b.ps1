...
$compiler = (Resolve-Path "$PSScriptRoot\gen3_w5b.exe").Path
...
$tests = @(
    ...
    'error_test','tuple_test'
)

$pass = 0; $fail = 0; $skip = 0; $failures = @()

Write-Host "=== W5b SANITY CHECK (gen3_w5b.exe) ==="
...
foreach ($t in $tests) {
    ...
    if (!(Test-Path $nova)) { Write-Host "SKIP $t (no .nova)"; $skip++; continue }
...
Write-Host ""
Write-Host "=== SANITY: $pass PASS, $fail FAIL, $skip SKIP ==="
