# Verify the TRACK-A fix BEFORE reconverge: build gen4 from the edited nova_compiler.nova, then
# run the repro (tka_xmod_test: unannotated cross-module call -> per-struct field slot) plus the
# existing cross-module / shared-field-name tests to confirm no regression. Kill-on-timeout.
...
$fail = 0
foreach ($t in @("tka_xmod_test","cross_module_struct_test","forge_typed_core_test","from_json_safety_test","auto_json_test")) {
...
Write-Host ""
if ($fail -eq 0) { Write-Host "=== TRACK-A fix VERIFIED (repro + no regression) ===" } else { Write-Host "=== FAILED ==="; exit 1 }
