...
& clang "$dir\nova_compiler.ll" $rtSrc -o "$dir\gen4_new.exe" -O2 @linkFlags 2>"$dir\_alg_lerr.txt"
...
Copy-Item "$dir\nova_compiler.ll" "$dir\_gen5_alg.ll" -Force
$sha5 = (Get-FileHash "$dir\_gen5_alg.ll" -Algorithm SHA256).Hash
Write-Host "gen5 SHA: $sha5"
& clang "$dir\_gen5_alg.ll" $rtSrc -o "$dir\gen5_alg.exe" -O2 @linkFlags 2>"$dir\_alg_lerr2.txt"
...
$psi3.FileName = "$dir\gen5_alg.exe"; $psi3.Arguments = "nova_compiler.nova"; $psi3.WorkingDirectory = $dir
...
foreach ($test in @("_algebra_test", "_unaryfold_test", "_bitfold_test", "_divmod_test", "_strength_test")) {
    Write-Host "--- $test ---"
    ...
    & clang "$dir\$test.ll" $rtSrc -o "$dir\$test.exe" -O2 @linkFlags 2>"$dir\_alg_tlerr.txt"
