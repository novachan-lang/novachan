...
if (!(Test-Path "gen2_trait.exe")) { Write-Host "FAIL: gen2_trait.exe not found"; exit 1 }

if (Test-Path "float_test.ll") { Remove-Item "float_test.ll" -Force }
$p = Start-Process -FilePath ".\gen2_trait.exe" -ArgumentList "float_test.nova" -WorkingDirectory $PSScriptRoot -RedirectStandardOutput "ft_cout.txt" -RedirectStandardError "ft_cerr.txt" -PassThru -NoNewWindow
