...
static int  nova_int_str_cache_inited = 0;
...
int64_t nova_rt_dict_set(int64_t handle, int64_t key, int64_t val) {
    ...
    uint64_t h;
    const char* k = nova_intern_h((const char*)(uintptr_t)key, &h);
    int64_t interned_key = (int64_t)(uintptr_t)k;
    int64_t slot = (int64_t)(h & (uint64_t)(d->idx_cap - 1));
    while (d->idx[slot] != DICT_IDX_EMPTY) {
        int64_t ei = d->idx[slot];
        if (d->hashes[ei] == h && (const char*)(uintptr_t)d->keys[ei] == k) {
    ...
    d->keys[d->size] = interned_key;
    d->vals[d->size] = val;
    d->hashes[d->size] = h;
    nova_rc_inc(interned_key);
    nova_rc_inc(val);
    d->idx[slot] = d->size;
    d->size++;
    dict_maybe_grow(d);
...
int64_t nova_rt_dict_has(int64_t handle, int64_t key) {
    ...
    while (d->idx[slot] != DICT_IDX_EMPTY) {
        int64_t ei = d->idx[slot];
        if (d->hashes[ei] == h && (const char*)(uintptr_t)d->keys[ei] == k) return 1;
        slot = (slot + 1) & (d->idx_cap - 1);
    }
    return 0;
}
...
int64_t nova_rt_str_concat_safe(int64_t a, int64_t b) {
    ...
    if (ta != NOVA_MEM_RAW && ta != (NovaMemTag)-1) sa = nova_rt_any_to_str(sa);
    if (tb != NOVA_MEM_RAW && tb != (NovaMemTag)-1) sb = nova_rt_any_to_str(sb);
    const char* ca = (const char*)(uintptr_t)sa;
    const char* cb = (const char*)(uintptr_t)sb;
    size_t la = strlen(ca), lb = strlen(cb);
    char* result = malloc(la + lb + 1);
    if (!result) return sa;
    memcpy(result, ca, la);
    memcpy(result + la, cb, lb + 1);
    nova_mem_track(result, NOVA_MEM_RAW);
    return (int64_t)(uintptr_t)result;
}
