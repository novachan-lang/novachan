...
$r = Invoke-Timed -FilePath (Resolve-Path '.\gen3_test.exe').Path -Arguments '_trail_fn_probe.nova' -TimeoutMs 60000 -WorkingDirectory $PSScriptRoot
Write-Host "COMPILE: $($r.ExitCode)"
if ($r.StdErr) { Write-Host $r.StdErr }
if ($r.ExitCode -ne 0) { exit 1 }
$lr = Invoke-Timed -FilePath 'clang' -Arguments '_trail_fn_probe.ll output/nova_runtime.o -o _trail_fn_probe.exe -O2 -lws2_32 -ladvapi32 -D_CRT_SECURE_NO_WARNINGS -w' -TimeoutMs 60000 -WorkingDirectory $PSScriptRoot
