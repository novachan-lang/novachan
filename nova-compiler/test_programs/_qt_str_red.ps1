...
foreach ($test in @("_strength_test", "_where_test", "_where_multi_test", "_loop_test", "_forelse_test")) {
    ...
    if (-not (Test-Path "$dir\$test.ll")) { Write-Host "$test COMPILE FAILED"; if ($ce) { Write-Host "  $ce" }; continue }
    & clang "$dir\$test.ll" $rtSrc -o "$dir\$test.exe" -O2 @linkFlags 2>"$dir\_sr_lerr.txt"
    ...
    $o2 = $p2.StandardOutput.ReadToEnd(); $p2.StandardError.ReadToEnd() | Out-Null
