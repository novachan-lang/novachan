...
$tests = @(
    "fiber_test","fiber_nested_test","sched_test","ws_sched_test",
    "spawn_test","spawn_multi_test","spawn_bidir_test","spawn_fanin_test","spawn_compute_test",
    "move_spawn_test","bounded_chan_test","select_test","select_multi_test",
    "monitor_test","monitor_multi_test","green_monitor_test","green_supervisor_test",
    "supervisor_test","supcrash_test","atomicx_test","parallel_test","async_test","t8_channel_test"
)
...
foreach ($test in $tests) {
    ...
    if (-not $ex2) { try { $pr2.Kill() } catch {}; Write-Host "$test RUN_TIMEOUT (likely deadlock)"; $fail++; continue }
    if ($pr2.ExitCode -ne 0) { Write-Host "$test FAIL (exit=$($pr2.ExitCode)) $($e2.Trim())"; $fail++ }
    else { Write-Host "$test PASS"; $pass++ }
}
Write-Host ""
Write-Host "Concurrency: $pass PASS, $fail FAIL, $skip skip"
