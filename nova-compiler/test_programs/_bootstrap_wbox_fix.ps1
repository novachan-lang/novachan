...
# Test the previously-crashing programs
...
foreach ($t in $tests) {
    ...
    $rt = Invoke-Timed "$dir\$t.exe" "" "${t}_run" 10000 $dir
    if ($rt.exit -eq 0) { Write-Host "$t : PASS" } else {
        Write-Host "$t : FAIL exit=$($rt.exit)"
        if ($rt.err.Length -gt 0) { Write-Host "  ERR: $($rt.err.Substring(0, [Math]::Min(200, $rt.err.Length)))" }
    }
}
