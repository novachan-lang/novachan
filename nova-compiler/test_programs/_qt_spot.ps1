...
if ($ce) { Write-Host "COMPILE ERR: $ce" }
if (-not (Test-Path "$dir\$test.ll")) { Write-Host "NO LL FILE"; exit 1 }
Write-Host "Compiled OK"

& clang "$dir\$test.ll" $rtSrc -o "$dir\$test.exe" -O2 @linkFlags 2>"$dir\_spt_lerr.txt"
