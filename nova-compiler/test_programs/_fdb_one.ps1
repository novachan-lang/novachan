# Parameterized forge_db smoke: compile <test>.nova (gen3) -> link runtime.o + sqlite3.o -> run. Kill-on-timeout.
Set-Location $PSScriptRoot
. "$PSScriptRoot\_proc_util.ps1"
if (-not $ClangPath) { $ClangPath = "clang" }
$gen3 = Join-Path $PSScriptRoot "gen3_test.exe"
$t = $args[0]
if (-not $t) { Write-Host "usage: _fdb_one.ps1 <testname-no-ext>"; exit 1 }
Invoke-Timed -FilePath $ClangPath -Arguments "-c -O2 output\nova_runtime.c -o output\nova_runtime.o -w" -TimeoutMs 180000 -WorkingDirectory $PSScriptRoot | Out-Null
