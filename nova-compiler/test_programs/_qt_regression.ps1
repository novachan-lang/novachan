...
if (-not (Test-Path $rtObj)) {
    & clang -c $rtSrc -o $rtObj -O2 -D_CRT_SECURE_NO_WARNINGS -w
}
...
Get-ChildItem "$dir\*.nova" | ForEach-Object {
    ...
    $rok = $r.WaitForExit(30000)
    if (-not $rok) { $r.Kill(); $fail++; $failures += "$base (RUN TIMEOUT)"; return }
    if ($r.ExitCode -ne 0) { $fail++; $failures += "$base (exit=$($r.ExitCode))"; return }
