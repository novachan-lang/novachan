...
$l1 = Invoke-Timed -FilePath $ClangPath -Arguments "-O2 -o `"$dir\gen4_rmm.exe`" `"$dir\nova_compiler.ll`" `"$dir\output\nova_runtime.c`" $NovaLinkFlags -D_CRT_SECURE_NO_WARNINGS -w" -TimeoutMs 150000
if (!(Test-Path "$dir\gen4_rmm.exe")) { Write-Host "LINK-FAIL gen4 $($l1.StdErr)"; exit 1 }
Write-Host "  gen4_rmm.exe built"
Write-Host "[2] gen4 -> compile remote_multi_test.nova"
$c2 = Invoke-Timed -FilePath "$dir\gen4_rmm.exe" -Arguments "remote_multi_test.nova" -TimeoutMs 60000
if ($c2.ExitCode -ne 0) { Write-Host "COMPILE-FAIL set_comp exit=$($c2.ExitCode)"; Write-Host $c2.StdErr; exit 1 }
$l2 = Invoke-Timed -FilePath $ClangPath -Arguments "-O2 -o `"$dir\remote_multi_test.exe`" `"$dir\remote_multi_test.ll`" `"$dir\output\nova_runtime.c`" $NovaLinkFlags -D_CRT_SECURE_NO_WARNINGS -w" -TimeoutMs 120000
if (!(Test-Path "$dir\remote_multi_test.exe")) { Write-Host "LINK-FAIL set_comp $($l2.StdErr)"; exit 1 }
Write-Host "[3] run remote_multi_test"
$r = Invoke-Timed -FilePath "$dir\remote_multi_test.exe" -TimeoutMs 15000
