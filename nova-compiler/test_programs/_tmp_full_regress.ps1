...
# Build new compiler from fixed source
...
Write-Host "Compiler built: $((Get-Item gen2_new.exe).Length) bytes"
...
foreach ($t in $regTests) {
    ...
    $cr2 = Invoke-Timed -FilePath $compiler -Arguments "$t.nova" -TimeoutMs 30000
    if ($cr2.ExitCode -ne 0) { Write-Host "FAIL $t : COMPILE"; $fail++; continue }
    $lr2 = Invoke-Timed -FilePath $ClangPath -Arguments "-O2 -o $t.exe $t.ll nova_runtime.c $NovaLinkFlags" -TimeoutMs 30000
...
foreach ($name in $stests.Keys) {
    ...
    $cr3 = Invoke-Timed -FilePath $compiler -Arguments "$name.nova" -TimeoutMs 30000
    if ($cr3.ExitCode -ne 0) { Write-Host "FAIL $name : compile"; $sf++; Remove-Item "$name.nova" -Force -ErrorAction SilentlyContinue; continue }
    $lr3 = Invoke-Timed -FilePath $ClangPath -Arguments "-O2 -o $name.exe $name.ll nova_runtime.c $NovaLinkFlags" -TimeoutMs 30000
    ...
    $rr2 = Invoke-Timed -FilePath (Resolve-Path ".\$name.exe").Path -Arguments "" -TimeoutMs 10000
    if ($rr2.ExitCode -ne 0) {
        Write-Host "FAIL $name : exit=$($rr2.ExitCode) out=$($rr2.StdOut)"
        $sf++
    } else {
        Write-Host "PASS $name : $($rr2.StdOut.Trim())"
        $sp++
    }
