...
& clang "$dir\nova_compiler.ll" $rtSrc -o "$dir\gen4_new.exe" -O2 @linkFlags 2>"$dir\_vm_lerr.txt"
if ($LASTEXITCODE -ne 0) { Write-Host "gen4 LINK FAILED"; Get-Content "$dir\_vm_lerr.txt" | Select-Object -First 20; exit 1 }

Write-Host "=== gen4 -> gen5 ==="
Invoke-Gen "$dir\gen4_new.exe" "gen4"
Copy-Item "$dir\nova_compiler.ll" "$dir\_gen5_vm.ll" -Force
$sha5 = (Get-FileHash "$dir\_gen5_vm.ll" -Algorithm SHA256).Hash
Write-Host "gen5 SHA: $sha5"
& clang "$dir\_gen5_vm.ll" $rtSrc -o "$dir\gen5_vm.exe" -O2 @linkFlags 2>"$dir\_vm_lerr2.txt"
if ($LASTEXITCODE -ne 0) { Write-Host "gen5 LINK FAILED"; exit 1 }

Write-Host "=== gen5 -> gen6 ==="
Invoke-Gen "$dir\gen5_vm.exe" "gen5"
...
if ($sha5 -eq $sha6) {
    ...
    Write-Host "DIVERGED (gen5 != gen6) -- NOT installing"
    exit 1
}

$compiler = "$dir\gen4_test.exe"
Write-Host ""
foreach ($test in @("value_model_golden", "_vm_float_broad")) {
    ...
    if (-not $ex) { try { $pr.Kill() } catch {}; Write-Host "$test COMPILE TIMEOUT"; continue }
    if (-not (Test-Path "$dir\$test.ll")) { Write-Host "$test COMPILE FAIL: $co $ce"; continue }
    & clang "$dir\$test.ll" $rtSrc -o "$dir\$test.exe" -O2 @linkFlags 2>"$dir\_vm_tlerr.txt"
    if ($LASTEXITCODE -ne 0) { Write-Host "$test LINK FAIL"; continue }
    ...
    if (-not $ex2) { try { $pr2.Kill() } catch {}; Write-Host "$test RUN TIMEOUT"; continue }
    Write-Host $o2.Trim()
