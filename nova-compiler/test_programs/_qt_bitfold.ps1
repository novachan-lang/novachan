...
& clang "$dir\nova_compiler.ll" $rtSrc -o "$dir\gen4_new.exe" -O2 @linkFlags 2>"$dir\_bf_lerr.txt"
...
Copy-Item "$dir\nova_compiler.ll" "$dir\_gen5_bf.ll" -Force
$sha5 = (Get-FileHash "$dir\_gen5_bf.ll" -Algorithm SHA256).Hash
Write-Host "gen5 SHA: $sha5"
& clang "$dir\_gen5_bf.ll" $rtSrc -o "$dir\gen5_bf.exe" -O2 @linkFlags 2>"$dir\_bf_lerr2.txt"
...
$psi3.FileName = "$dir\gen5_bf.exe"; $psi3.Arguments = "nova_compiler.nova"; $psi3.WorkingDirectory = $dir
...
foreach ($test in @("_bitfold_test", "_divmod_test", "_strength_test")) {
    ...
    & clang "$dir\$test.ll" $rtSrc -o "$dir\$test.exe" -O2 @linkFlags 2>"$dir\_bf_tlerr.txt"
