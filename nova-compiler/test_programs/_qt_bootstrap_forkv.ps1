...
if (-not (Test-Path "$dir\nova_compiler.ll")) {
    Write-Host "nova_compiler.ll not found, compiling with gen3..."
    ...
    if (-not (Test-Path "$dir\nova_compiler.ll")) { Write-Host "gen3 COMPILE FAILED"; exit 1 }
}
...
if ($e2) { Write-Host "gen4 STDERR: $e2" }
if (-not (Test-Path "$dir\nova_compiler.ll")) { Write-Host "gen4 COMPILE FAILED"; Write-Host $o2; exit 1 }
Copy-Item "$dir\nova_compiler.ll" "$dir\gen5.ll" -Force
Write-Host "gen5.ll produced"

& clang "$dir\gen5.ll" $rtSrc -o "$dir\gen5.exe" -O2 @linkFlags 2>"$dir\_bs_lerr.txt"
if ($LASTEXITCODE -ne 0) { Write-Host "gen5 LINK FAILED"; Get-Content "$dir\_bs_lerr.txt"; exit 1 }
...
if ($e3) { Write-Host "gen5 STDERR: $e3" }
if (-not (Test-Path "$dir\nova_compiler.ll")) { Write-Host "gen5 COMPILE FAILED"; Write-Host $o3; exit 1 }
Copy-Item "$dir\nova_compiler.ll" "$dir\gen6.ll" -Force
Write-Host "gen6.ll produced"

Write-Host ""
Write-Host "=== Step 4: SHA256 convergence check ==="
...
if ($h5 -eq $h6) {
    ...
    Write-Host "DIVERGED - bootstrap failed"
    exit 1
}
