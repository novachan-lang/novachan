...
foreach ($mode in @('default','track8','autoarena')) {
    ...
    $r3 = Invoke-Timed -FilePath "$PSScriptRoot\bench_track8.exe" -Arguments "" -TimeoutMs 30000 -WorkingDirectory $PSScriptRoot
    Write-Host "[$mode] stdout: $($r3.StdOut.Trim())"
    Write-Host "[$mode] stderr: $($r3.StdErr.Trim())"
}
Remove-Item "$PSScriptRoot\bench_track8.exe","$PSScriptRoot\bench_track8.ll" -Force -ErrorAction SilentlyContinue
$env:NOVA_TRACK8 = $null
$env:NOVA_AUTO_ARENA = $null
