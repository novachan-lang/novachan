...
Write-Host ""
Write-Host "=== Compiling type_pred_test.nova ==="
$ll = "$base\type_pred_test.ll"
if (Test-Path $ll) { Remove-Item $ll -Force }
$env:NOVA_NO_CACHE = "1"
$r3 = Invoke-Timed -FilePath "$base\workerx.exe" -Arguments "`"$base\type_pred_test.nova`"" -TimeoutMs 60000 -WorkingDirectory $base
$env:NOVA_NO_CACHE = $null
...
$r5 = Invoke-Timed -FilePath "$base\type_pred_test.exe" -Arguments "" -TimeoutMs 15000 -WorkingDirectory $base
Write-Host ""
Write-Host "=== RUN ==="
