...
function createRuntime() {
  ...
  // The compiler's print specializer (nova_compiler.nova ~20670) rewrites a generic
  // nova_rt_print_any call into the type-specific variant below once inference proves
  // the operand's type. All five must exist here or the wasm module fails to link.
  ...
  function nova_rt_print_int(val) {
    return emitLine(BigInt.asIntN(64, val).toString());
  }

  function nova_rt_print_bool(val) {
    return emitLine(val ? "true" : "false");
  }

  function nova_rt_print_float(bits) {
    return emitLine(readCStr(Number(nova_rt_float_to_str(bits) & 0xFFFFFFFFn)));
  }

  function nova_rt_print_any(val) {
    return emitLine(readCStr(Number(nova_rt_any_to_str(val) & 0xFFFFFFFFn)));
  }

  function nova_rt_print_intlist(listI64) {
    return emitLine(readCStr(Number(nova_rt_intlist_to_str(listI64) & 0xFFFFFFFFn)));
  }

  // Mirrors the native nova_rt_intlist_to_str: "[a, b, c]", elements as raw i64 decimals.
  function nova_rt_intlist_to_str(listI64) {
    const lp = Number(listI64 & 0xFFFFFFFFn);
    if (lp === 0) return BigInt(writeCStr("[]"));
    if (dv.buffer !== memory.buffer) refreshViews();
    const size = dv.getBigInt64(lp + 8, true);
    const dp = dv.getUint32(lp, true);
    const parts = [];
    for (let i = 0n; i < size; i++) {
      parts.push(dv.getBigInt64(dp + Number(i) * 8, true).toString());
    }
  ...
  // CORE_GAP 0.8 (native, landed after this runtime's original m6 milestone): ordinary
  // (non-repr_c, non-over-aligned) structs now allocate via nova_rt_hashed_struct_alloc
  // instead of nova_rt_struct_alloc, so nova_rc_free can consult a per-type managed-slot
  // bitmap on free. That distinction is a native RC-bookkeeping concern only -- slot 0's
  // type-hash value is written by a plain `store` the compiler emits directly into the
  // returned block, not by this function. This tag-free bump allocator draws no
  // distinction between the two allocation kinds, so the same bump allocation serves both.
  function nova_rt_hashed_struct_alloc(sizeI64) {
    return bumpAlloc(Number(sizeI64));   // i32 (Number)
  }
  ...
  // CORE_GAP 0.2 (native, landed after this runtime's original m1-m6 milestones): int
  // division/modulo used to be a raw sdiv/srem instruction (UB on x/0 and INT64_MIN/-1).
  // The compiler now routes int-typed AND untyped/any-typed div/mod through these calls
  // so both cases are DEFINED (x/0 = 0, INT64_MIN/-1 wraps) instead of a trap. A
  // statically float-typed operand never reaches here -- that codegen path emits a raw
  // `fdiv`/`frem` instead (see nova_compiler.nova's `tk == "float"` branch), so these two
  // functions only ever see genuine int operands in the traffic this coverage runtime
  // can produce. NOTE: native additionally handles a genuinely-`any`-typed operand that
  // turns out to be a BOXED float at runtime (nova_is_likely_float) -- this tag-free
  // runtime has no boxed-float representation to detect (see nova_rt_to_float above), so
  // that one native branch is unreachable here; it is the same class of residual gap
  // already documented for nova_rt_add/any_to_str/contains, not a new one.
  function nova_rt_div(a, b) {
    if (b === 0n) return 0n;                          // x/0 defined 0, never UB
    if (b === -1n) return BigInt.asIntN(64, -a);       // INT64_MIN/-1 wraps, never UB
    return a / b;                                       // BigInt / truncates toward zero, matching C
  }
  function nova_rt_mod(a, b) {
    if (b === 0n) return 0n;
    if (b === -1n) return 0n;
    return a % b;                                        // BigInt % matches C's truncated-division remainder
  }

  // Typed-array / raw-float-list support (S4 kind=2 packed float lists) is out of scope
  // for this coverage runtime -- every list this runtime builds is the plain generic i64-
  // element kind, so it is never a kind2 list. Always-false is therefore sound, not a guess.
  function nova_rt_list_is_kind2(handle) { return 0n; }
  // Native's own implementation is `return handle;` verbatim (a view over an existing
  // kind2 list's raw buffer needs no separate object) -- identity here matches exactly,
  // not a simplification.
  function nova_rt_floatlist_view(handle) { return handle; }
  // Typed-float element read (S4.2). Native branches on elem_kind: kind=2 returns the raw
  // double bits directly, kind=0 unboxes a boxed float. Every list this runtime builds is
  // kind=0-equivalent (never kind2, per list_is_kind2 above) AND this runtime never boxes
  // floats separately from raw i64 values (nova_rt_to_float returns raw bits, not a boxed
  // pointer -- see its comment above), so "unbox" is already an identity here, exactly
  // like nova_rt_unbox_elem. list_get_f therefore reduces to exactly list_get.
  function nova_rt_list_get_f(listI64, index) { return nova_rt_list_get(listI64, index); }

  // Native boxes a float as a separate heap object (NovaBox{kind:FLOAT, payload:bits}) so
  // find_tag/any-dispatch can recognize "this i64 is a float" rather than an int. This
  // coverage runtime has never had that representation -- every float here is the raw
  // IEEE-754 bit pattern, passed around exactly like an int (see nova_rt_to_float/
  // nova_rt_unbox_elem above, both already identity for the same reason). Box/unbox are
  // therefore identity too: nothing in this runtime ever allocates or checks for a box, so
  // there is no real object for box to wrap or unbox to unwrap. This is the same documented
  // soundness boundary as the rest of the file (no value tags), not a new gap: a program
  // that never mixes an `any`-typed float with an `any`-typed medium/large int through a
  // polymorphic op is unaffected either way.
  function nova_rt_box_float(bits) { return bits; }
  function nova_rt_unbox(handle) { return handle; }

  function nova_rt_list_get(listI64, index) {
    const lp = Number(listI64 & 0xFFFFFFFFn);
    if (dv.buffer !== memory.buffer) refreshViews();
    const size = dv.getBigInt64(lp + 8, true);
    let idx = index;
    if (idx < 0n) idx += size;
    if (idx < 0n || idx >= size) return 0n;   // native sets an error flag we don't track; 0 matches index_get's OOB convention
    const dp = dv.getUint32(lp, true);
    return dv.getBigInt64(dp + Number(idx) * 8, true);
  }

  // -- RC companions (NOVA_T8_FULLRC, default-on since ~2026-07): the native runtime
  // rc_dec's the value a slot/temporary previously held once the compiler's escape
  // analysis proves it uniquely-owned and never-escaped -- the fix for the headline
  // loop-rebind leak (project_waveb6_rc_leak_real_diagnosis). This bump-allocator
  // runtime never frees ANYTHING (see file header), so both are sound no-ops: there
  // is nothing to double-free and no observable difference in a single wasm run.
  function nova_rt_rc_drop_reassign(oldv, newv) { return 0n; }
  function nova_rt_rc_drop_temp(v) { return 0n; }

  // -- Explicit frees --
  // Lists live in linear memory (bump-allocated, never individually freeable) --
  // list_free_local is a genuine no-op here, same reasoning as the RC companions.
  function nova_rt_list_free_local(listI64) { return 0n; }
  // Dicts are JS-side state (a Map entry keyed by handle, see "Dicts" above), so unlike
  // lists we CAN honor a free: the compiler only emits this call when its escape
  // analysis proved the handle uniquely-owned and dead after this point, so dropping
  // the Map entry is sound and actually reclaims JS heap (useful for dict-per-iteration
  // loops). A later use of the same handle behaves like native's freed-dict UB class:
  // isDictHandle() will (correctly) report false, so callers see "not a dict" rather
  // than resurrecting stale data.
  function nova_rt_dict_free_local(handle) { dicts.delete(handle); return 0n; }

  // -- Software stack-overflow guard (wasm/freestanding-only; landed 2026-08-04) --
  // Native wasm/freestanding targets have neither hardware guard pages nor SEH, so the
  // compiler auto-emits stack_enter/stack_exit at every function's entry/return as the
  // ONLY overflow containment, turning an opaque engine trap into a clean panic. This
  // JS host runs each wasm call as a real JS call, so V8's OWN stack limit already
  // catches unbounded recursion (a RangeError, caught by runFile's catch-all) -- a
  // second counter here would duplicate that protection without changing the outcome
  // (a program that overflows still fails loudly, just with a JS RangeError instead of
  // NOVA's "stack overflow (depth > N)" panic text). No-op, consistent with the other
  // RC/free companions: this coverage runtime does not attempt native's exact panic
  // message fidelity, only sound execution.
  function nova_rt_stack_enter() { return; }
  function nova_rt_stack_exit() { return; }
  function nova_rt_stack_set_max(maxI64) { return; }
  ...
  const imports = {
    ...
    nova_rt_print_float,
    nova_rt_print_any,
    nova_rt_print_intlist,
    nova_rt_intlist_to_str,
    nova_rt_to_float,
    nova_rt_float_to_str,
    nova_rt_struct_alloc,
    nova_rt_hashed_struct_alloc,
    nova_rt_dict_create,
    nova_rt_dict_set,
    ...
    nova_rt_contains,
    nova_rt_list_get,
    nova_rt_rc_drop_reassign,
    nova_rt_rc_drop_temp,
