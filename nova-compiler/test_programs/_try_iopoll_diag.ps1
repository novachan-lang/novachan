...
$ps.FileName = $compiler; $ps.Arguments = "_iopoll_diag.nova"; $ps.WorkingDirectory = $dir
...
& clang "$dir\_iopoll_diag.ll" $rtObj -o "$dir\_iopoll_diag.exe" -O2 -lws2_32 -ladvapi32 -D_CRT_SECURE_NO_WARNINGS -w 2>&1
if (-not (Test-Path "$dir\_iopoll_diag.exe")) { Write-Host "LINK FAIL"; exit 1 }

$rps = New-Object System.Diagnostics.ProcessStartInfo
$rps.FileName = "$dir\_iopoll_diag.exe"; $rps.WorkingDirectory = $dir
...
if (-not $rpr.WaitForExit(15000)) { $rpr.Kill(); $rpr.WaitForExit(3000); Write-Host "TIMEOUT" }
[System.Threading.Tasks.Task]::WaitAll($rout, $rerr)
Write-Host "Exit: $($rpr.ExitCode)"
Write-Host $rout.Result
if ($rerr.Result.Length -gt 0) { Write-Host "STDERR: $($rerr.Result)" }
