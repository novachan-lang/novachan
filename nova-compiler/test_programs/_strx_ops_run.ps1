...
Remove-Item strx_ops_test.ll, __sx.exe -Force -ErrorAction SilentlyContinue
$c = Invoke-Timed -FilePath '.\gen4_test.exe' -Arguments 'strx_ops_test.nova' -TimeoutMs 60000 -WorkingDirectory $PSScriptRoot
...
if (!(Test-Path strx_ops_test.ll)) { Write-Host "NO .ll"; exit 1 }
$link = "-O2 -o __sx.exe strx_ops_test.ll output\nova_runtime.o -lws2_32 -ladvapi32 -D_CRT_SECURE_NO_WARNINGS -w"
...
$r = Invoke-Timed -FilePath '.\__sx.exe' -Arguments '' -TimeoutMs 15000 -WorkingDirectory $PSScriptRoot
