...
$tests = @('float_test','list_test','string_test','closure_test','match_test','struct_test','dict_test','for_test','enum_test','range_test','tuple_test','defaults_test','named_args_test','comprehension_test','match_guard_test','method_test','destructure_test','match_expr_test','bitwise_test','yield_test','error_test','for_continue_test','generics_test','trait_test')
...
foreach ($t in $tests) {
    ...
    $linkArgs = "-O2 -o `"$t.exe`" `"$t.ll`" `"$runtimeSrc`" $NovaLinkFlags -D_CRT_SECURE_NO_WARNINGS -w"
