...
Remove-Item _tc_stress.ll, __tcs.exe -Force -ErrorAction SilentlyContinue
$c = Invoke-Timed -FilePath '.\gen4_test.exe' -Arguments '_tc_stress.nova' -TimeoutMs 60000 -WorkingDirectory $PSScriptRoot
if ($c.ExitCode -ne 0) { Write-Host "COMPILE FAIL exit=$($c.ExitCode)"; if ($c.StdOut) { Write-Host $c.StdOut }; exit 1 }
if (!(Test-Path _tc_stress.ll)) { Write-Host "NO .ll"; exit 1 }
$link = "-O2 -o __tcs.exe _tc_stress.ll output\nova_runtime.o -lws2_32 -ladvapi32 -D_CRT_SECURE_NO_WARNINGS -w"
...
$r = Invoke-Timed -FilePath '.\__tcs.exe' -Arguments '' -TimeoutMs 30000 -WorkingDirectory $PSScriptRoot
