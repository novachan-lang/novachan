...
& clang "$dir\nova_compiler.ll" $rtSrc -o "$dir\gen4_new.exe" -O2 @linkFlags 2>"$dir\_uf_lerr.txt"
...
Copy-Item "$dir\nova_compiler.ll" "$dir\_gen5_uf.ll" -Force
$sha5 = (Get-FileHash "$dir\_gen5_uf.ll" -Algorithm SHA256).Hash
Write-Host "gen5 SHA: $sha5"
& clang "$dir\_gen5_uf.ll" $rtSrc -o "$dir\gen5_uf.exe" -O2 @linkFlags 2>"$dir\_uf_lerr2.txt"
...
$psi3.FileName = "$dir\gen5_uf.exe"; $psi3.Arguments = "nova_compiler.nova"; $psi3.WorkingDirectory = $dir
...
foreach ($test in @("_unaryfold_test", "_bitfold_test", "_divmod_test", "_strength_test")) {
    ...
    if (-not (Test-Path "$dir\$test.ll")) { Write-Host "$test COMPILE FAIL: $ce"; exit 1 }
    & clang "$dir\$test.ll" $rtSrc -o "$dir\$test.exe" -O2 @linkFlags 2>"$dir\_uf_tlerr.txt"
    ...
    Write-Host $o2.Trim()
    if ($pr2.ExitCode -ne 0) { Write-Host "$test FAILED exit=$($pr2.ExitCode)"; exit 1 }
