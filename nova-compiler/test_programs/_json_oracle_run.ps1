...
Remove-Item json_oracle_test.ll, __jo.exe -Force -ErrorAction SilentlyContinue
$c = Invoke-Timed -FilePath '.\gen4_test.exe' -Arguments 'json_oracle_test.nova' -TimeoutMs 60000 -WorkingDirectory $PSScriptRoot
...
if (!(Test-Path json_oracle_test.ll)) { Write-Host "NO .ll"; exit 1 }
$link = "-O2 -o __jo.exe json_oracle_test.ll output\nova_runtime.o -lws2_32 -ladvapi32 -D_CRT_SECURE_NO_WARNINGS -w"
...
$r = Invoke-Timed -FilePath '.\__jo.exe' -Arguments '' -TimeoutMs 15000 -WorkingDirectory $PSScriptRoot
