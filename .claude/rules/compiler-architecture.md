...
3. **The compiler must be self-hosting eventually.** Every design choice should consider: can NOVA express this about itself? If the compiler requires features NOVA can't express, the language has a gap.
...
## Code Quality Standards for Compiler Kotlin Code

1. **No stringly-typed IR.** LLVM IR is emitted as text, but every `appendLine` must be verified: correct types, correct alignment, correct calling convention. Read the LLVM LangRef for every instruction you emit. Incorrect IR is not "a bug to fix later" — it's UB in the final binary.

2. **Type tracking must be complete.** `refTypes` map must be updated for EVERY instruction that produces a value. A missing entry means downstream code guesses the type — and guesses wrong. The `emitBinary ADD` segfault (Any+Any→str_concat_safe) was caused by exactly this.
