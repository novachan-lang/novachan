...
**Still genuinely open (accurate, post-verification 2026-07-02):** the remaining items are now NARROW and almost entirely compiler/runtime. VERIFIED DONE this session (struck from "open"): `nova new` scaffolder (B9, live), HTTP/2 **HPACK incl. Huffman** + frame codec + flow control + **h2c cleartext serve** (D2.2–D2.5 over cleartext, all `_fdb_one`-tested; a real RFC C.4.1 Huffman header block decodes end-to-end), and the **hero struct-insert with auto-assigned PK** (`orm_insert` id==0 skip). TRULY REMAINING (all gated): (1) HTTP/2 **ALPN-TLS** runtime hook (D2.1 — the one piece between h2c and h2-over-TLS; SChannel/OpenSSL, full `nova_ci`); (2) `with tx {}` compiler sugar (the function `with_tx` already works); (3) the `<T>` call-site SUGAR for db_insert/query (the RTTI capability already works); (4) **interfaces #8** (S6, compiler); (5) **WASM frontend** (S13); (6) `remote_spawn`-prod + mesh + cluster pubsub (S11, runtime); (7) LiveView render-differ completeness + GenServer `on_info`. gRPC/GraphQL/MQ/admin (S8/S10/S12) are shipped + tested; gRPC/GraphQL *serving* rides HTTP/2 (h2c works today; TLS needs ALPN).
...
| B9 | ~~`nova new` routes to a STUB~~ **RESOLVED (verified 2026-07-02)** | — | STALE. `cmd=="new"` in nova_compiler.nova (~L21506) calls the REAL scaffolder `nn_new_project` (all 5 kinds: --api/--microservice/--frontend/--fullstack/--lib), NOT the stub. Verified live: `gen3_test.exe new <name> --fullstack` scaffolds src/main.nova + nova.toml + public/ + README, and the generated app COMPILES. CLAUDE.md "download → full-stack app" promise is met. |
