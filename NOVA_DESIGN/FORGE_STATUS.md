...
**Ship Layer 2's `body_as<T>` / `query_as<T>` / `db.all<T>` typed wrappers.**

It is the *only* thing standing between the 22-line hero example and reality. The keystone underneath it (struct→JSON RTTI, `from_json_safe<T>`, the DB pool) is **already shipped and tested**; what's missing is the **let-site→callee specialization (Phase 0c)** that lets `let u = forge.body_as(req)` infer `T` from the binding, and the typed-row mapping that turns DB rows into structs via field-meta. (Honest caveat from §1: the *full* hero needs all of L2's typed wrappers — `all`/`find`/`insert`/`delete` + `body_as` — not just one. This item ships them as a set.) It is medium complexity, needs no new runtime, and `body_as` **must default to the safe Result path** (B5). It unblocks the entire "return your type → get JSON; bind the body → get your type" story that *is* Forge's beginner pitch, and it is the literal first impression a new developer gets. Everything else — OTP promotion, observability, HTTP/2 — is leverage; this is the front door. Build it next.
