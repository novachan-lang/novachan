...
**Sprint S7 — HTTP/2 (P4 flagship) — STARTED (2026-07-02):** greenfield (ALPN missing from the TLS runtime); building the pure-logic pieces first per NOVA_DESIGN/HTTP2_PLAN.md. **D2.2 HPACK §5.1 prefix-integer coding — ✅ (`eb2eb95`):** `forge_hpack` leaf module `hpack_encode_int`/`hpack_decode_int`, byte-exact vs RFC 7541 C.1 (10@5, 1337@5→1F 9A 0A, 42@8) + flags + boundary (`forge_hpack_test`). Next: §5.2 Huffman, static/dynamic tables, header-block codec, then the frame codec.
