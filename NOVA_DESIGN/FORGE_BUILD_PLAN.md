...
**Sprint S10 — L6/L7 comms (P3) — STARTED (reconciled 2026-07-01):** **D7r.1 outbound HTTP(S) client — ✅ (`f27cd53`):** `forge_http_client` (methods/headers/body, https verify-full, chunked de-framing, read-to-EOF, `http_request_to` read-deadline; `forge_http_client_test` loopback GET+POST). **D7r.2 resilience decorators — the "zoo = decorators over a call" thesis proven:** `retryx`/`circuitx`/`backoffx` (already built) + the client timeout compose over the outbound `Result` call (`forge_resilient_client_test`; commit pending its gate). Remaining P3: WS/SSE **consume** client, idempotency keys + If-Match OCC, OTLP/SMTP exporters.
