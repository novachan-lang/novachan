...
**Next in S1 (remaining = FOCUSED UNITS, not low-risk increments):** T2.7 `with tx { }` — a substantial NEW-SYNTAX feature (parser `with` production + panic-aware desugar: acquire/BEGIN/COMMIT-on-normal/ROLLBACK-on-panic/release), the FIRST S1 compiler-touching task → full reconverge gate. NOT a quick task. **Non-blocking**: the explicit-list `tx(pool, [[sql,params],...])` already gives transactions, so `with tx` is pure ergonomic sugar. T2.8 Postgres is env-gated (needs a local PG) AND depends on T2.7. **S1's marquee (hero) + every low-risk stdlib piece (T2.4/T2.5/T2.6/T2.9/T2.10) are DONE; what remains is one compiler feature (T2.7) + one env-gated driver (T2.8).** T2.2/T2.3 (`body_as`/`query_as`) ride T2.1's typed-let, already exercised by the hero.
