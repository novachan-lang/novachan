...
**Next in S1 (sequenced):** T2.7 `with tx { }` lexical sugar (compiler/parser desugar — the FIRST S1 compiler-touching task → full reconverge gate) → T2.10 `resp_model` nested fail-closed projection (`forge.nova`) → T2.8 Postgres (env-gated, needs a local PG). T2.2/T2.3 (`body_as`/`query_as` wrappers) ride T2.1's typed-let and are already exercised by the hero. **S1 marquee deliverable (the hero) is DONE; remaining S1 = hardening/extending the layer.**
