...
| **11** TLS server | `tls_listen/accept` + client `tls_connect` over OpenSSL (Linux/macOS) | **REAL ✓ (Linux, 2026-05-30) / Windows server TODO** | NOVA terminates TLS itself: server loads cert+key, real SSL_accept; client real SSL_connect. ORACLE: NOVA TLS server ↔ NOVA TLS client loopback with a self-signed cert → `pong-tls` over a real handshake (_tls_server_probe.ps1, built `-DNOVA_HAVE_OPENSSL`). Windows still client-only (schannel). |
| **11** distributed | length-framed message transport (node_*) + supervisor restart pattern | **REAL transport ✓ / FT supervision REAL ✓ (2026-05-30) / node-FT + isolation TODO** | _node_probe round-trip; supervisor_test: worker fails 2× → supervisor restarts → recovers (payload 102), 79/79. REMAINING: node-level heartbeat/reconnect/discovery; true OS-crash isolation = Track-8 process-ownership model. |
| **11** hot reload | file-change detection | **PARTIAL** | real mtime polling; no live code swap |
| **12** WASM **compile target** (NOVA→.wasm, browser/Node) | `gen3 compile --target wasm` → clang wasm32 → .wasm; runs in Node | **REAL ✓ (2026-05-30)** | NOVA→WASM→Node: a pure-compute program (`sum 1..100`) returns **5050** from the exported `nova_user_main` in Node (_wasm_node_probe.ps1 + nova_runtime_wasm.c + wasm_run.js). This is the **frontend leg** of full-stack-in-one-language. Compute-only (no IO/threads/net in the wasm sandbox). |
| **12** WASM interpreter | bytecode interpreter: **i32+i64+f32+f64** (const/arith/compare) + conversions + locals + **structured control flow** + **br_table** + **function calls & recursion** + **linear memory** (load/store 8/16/32, memory.size) | **REAL ✓ (i32/i64/f32/f64 + control + calls + memory, 2026-05-29)** | add→42; loop→55; fac(5)→120; mem→30; i64 10^12; trunc(3.5+1.5)→5. memory.grow/imports TODO |
...
| **TLS server** (`tls_listen/accept`) | — | **REAL ✓ Linux (2026-05-30)** | OpenSSL server: loads cert+key, SSL_accept; paired NOVA tls_connect client (SSL_connect). NOVA↔NOVA TLS loopback with self-signed cert returns `pong-tls` over a real handshake. Windows server-side (schannel AcceptSecurityContext) still TODO. |
...
| **HTTP client** (`http_get/post`) | done | **REAL ✓ cross-platform (http+https) 2026-05-30** | Windows: WinHTTP. Linux/macOS: native HTTP/1.1 over Berkeley sockets (`http://`) + **TLS via OpenSSL (`https://`)** with peer cert verification + SNI (built `-DNOVA_HAVE_OPENSSL -lssl -lcrypto`; default builds stay OpenSSL-free). ORACLES: http loopback `hello-nova!` (Linux+Windows); **https GET example.com over real TLS → "Example Domain"** (Linux/Docker, _https_client_probe.ps1); 78/78. |
