...
| **HTTP client** (`http_get/post`) | done | **REAL (http) ✓ cross-platform 2026-05-30 / PARTIAL (https on Linux)** | Windows: WinHTTP (http+https). Linux/macOS: native HTTP/1.1 over Berkeley sockets for `http://` (getaddrinfo+connect, header strip, returns body). ORACLE: NOVA `http_get` ↔ spawned NOVA server loopback → `hello-nova!` on BOTH Linux (Docker) and Windows; 78/78. REMAINING: `https://` on Linux/macOS (OpenSSL — next increment). |
