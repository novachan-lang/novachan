...
|---|---|---|---|---|
| **TLS** (`tls_connect/listen/send/recv`) | done | **STUB — DANGEROUS** | Wraps plaintext TCP, ignores cert/key (nova_runtime.c:9153 `(void)cert_path_val`). Sends unencrypted bytes while claiming "TLS". | Link a real TLS lib (OpenSSL/mbedTLS/schannel). Oracle: connect to `https://` real server; packet capture confirms ciphertext, not plaintext. |
...
| **WASM** (`wasm_compile/run/free`) | done | **STUB** | `wasm_run` loads bytes then `return 0` (nova_runtime.c:9373). Executes nothing. | Real WASM exec (embed wasm3/wasmtime, OR emit real .wasm from LLVM). Oracle: run wat2wasm-built fib(10), assert == 55. |
