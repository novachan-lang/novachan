...
| **12** WASM | bytecode interpreter: i32 const + locals + full compare/arith + **structured control flow** (block/loop/if/else/br/br_if/return/select) | **REAL ✓ (control flow added 2026-05-29)** | _wasm_probe: add→42; _wasm_loop_probe: real loop sum(1..10)→55. memory/calls/i64/f32/f64 TODO |
...
| **WASM** (`wasm_compile/run/free`) | done | **REAL ✓ (control flow added 2026-05-29)** | Real bytecode interpreter: LEB128 section parsing; i32.const; local.get/set/tee; the full i32 compare set (eqz/eq/ne/lt/gt/le/ge signed+unsigned) and arith/bitwise (add/sub/mul/div/rem/and/or/xor/shl/shr signed+unsigned); drop/select/nop; and **structured control flow** (block/loop/if/else/br/br_if/return) via a one-pass bracket-matching pre-scan. ORACLES: add→42 (_wasm_probe); a hand-crafted **loop** module summing 1..10 with locals+br_if → 55 (_wasm_loop_probe + wasm_loop_test.nova); 78/78 green. REMAINING: function calls (recursion), linear memory ops, i64/f32/f64, br_table. |
