...
| 2 ✅ | ARM/aarch64 fiber context switch — **FIXED `50cbf9f1`; concurrency works on ARM** | Platform | High | L | `nova_asm_switch` has no aarch64 branch and no `#else`. `spawn`/generators silently no-op on ARM. ✅ **TESTABLE HERE after all (2026-08-02): `docker run --platform linux/arm64` runs aarch64 under QEMU, and the runtime BUILDS AND RUNS there (object-space suite 8/8).** Correction to this row's wording: there IS an `#else` — it STUBS fibers. Measured on real ARM: `fiber_create` -> 0 + "fibers not supported on this platform", `fiber_resume` -> 1 (already-done, body never runs), generators empty. So the gap is confirmed by measurement, and the aarch64 `nova_asm_switch` can now be written AND verified on this machine. |
...
10. **ARM/aarch64 fiber context switch (L)** — ✅ **DONE `50cbf9f1`, verified on real aarch64.** (was: fully developable AND verifiable here via `docker run --platform linux/arm64` (QEMU); runtime already runs on aarch64, gap measured 2026-08-02.** Add the `nova_asm_switch` aarch64 branch. Concurrency silently
...
| MySQL FLOAT/DOUBLE/DATE binary decode returns "" (undecoded) | ✅ DONE | medium | S | Temporal ✅ `15b3546b` (DATE/DATETIME/TIMESTAMP/TIME → ISO). FLOAT/DOUBLE ✅ — added reusable `float_from_bits`/`f32_from_bits` reinterpret builtins (runtime + 6 compiler sites, reconverged) + `_my_bin_val` decodes via them. KATs `_mysql_temporal_test` + `_fbits_test`. **New primitive also unblocks `std/encoding/pack` floats** (the "bitcast f64<->i64 NOVA doesn't expose" note at pack.nova:34-35 is now stale).| Decode IEEE-754 LE 4/8-byte floats to a decimal string; decode the MySQL binary DATE/DATETIME/TIME layout to ISO. |
...
| Redis RESP bulk strings NUL-truncate on binary values | ✅ DONE | low | S | **forge_redis ✅ `9266fa52`** (full bytes-based rewrite, bulk as `bytes`) + **std/net/resp2 ✅ `a64c68de`** (`_r2_parse_bulk` exposes binary-safe `d["bytes"]` via `bytes_slice`; `d["str"]` is the text view). KATs `_redis_binsafe_test` + `_resp2_test` (0x00-in-bulk) both modes. | Assemble RESP bulk strings as byte lists; keep a string convenience wrapper for text. |
...
29. ✅/🔄 **L6 `let mut`** ✅ DONE `1a65d7c0` + **L8 custom index/iter** ✅ DONE `49f28f4f` + **L10 weak-ref/Drop** ✅ **BOTH NOW IMPLEMENTED (2026-08-03).** Weak refs were already there (`nova_rt_weak_create`/`_upgrade`/`_alive`/`_invalidate` + compiler builtins) — this row previously claimed NOT STARTED, which was wrong. **`Drop` is now implemented too:** a type declaring `fn T.drop(self)` gets its address registered in the struct metadata (`nova_rt_register_struct_drop`, emitted next to the redact/bitmap registrations), and `rc_free` invokes it BEFORE releasing the fields so the destructor can still read them — the same order Rust uses. The refcount is PINNED across the call, because a destructor is ordinary NOVA code that may take and release transient references to `self`; without the pin the matching `rc_dec` would drive the count back to zero and re-enter `rc_free` on an object already being destroyed. **LIMITATION, stated precisely: this gives DESTRUCTORS, not yet RAII.** The hook fires wherever the refcount actually reaches zero — reassignment, container destruction — but NOT at scope exit, because NOVA does not release locals when a function returns. Scope-exit release is a separate RC feature that was attempted before and reverted as unsound (see the `iter-88 Stage4 aggressive scope-exit RC` stash: "flag-on 399/33, 0xC0000005 UAF"). KAT `_kat_drop`.
