...
10. **ARM/aarch64 fiber context switch (L)** — ✅ **UNBLOCKED: fully developable AND verifiable here via `docker run --platform linux/arm64` (QEMU); runtime already runs on aarch64, gap measured 2026-08-02.** Add the `nova_asm_switch` aarch64 branch. Concurrency silently
