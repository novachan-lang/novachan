...
| **LOCK-1..12** | LOCK-3 ✅, **LOCK-4 ✅ (inc1..inc3d, 2026-08-04)**, **LOCK-11 ✅ (struct-by-value FFI, params + returns, 3 ABIs)** | LOCK-2 (Phase-1 hooks, wrong point) | LOCK-1, LOCK-5, LOCK-6, LOCK-7, LOCK-8, LOCK-9, LOCK-10, LOCK-12 | LOCK-4 — the #1 risk and the gate for 6 frameworks — is COMPLETE |
...
1. **LOCK-4 is COMPLETE (2026-08-04) — the gate for 6 frameworks is OPEN.** Every item in the old blocker list is closed: widths ride in `NType.name`/`IrType.name`; `255u8+1 == 0`; `let x: u8 = 300` narrows to 44; f32 is REAL binary32 (`16777217.0f32` -> 16777216.0, rounding after every operation) and f32-vs-f64 is a type error; packed sized arrays exist as a separate object kind (`std/collections/typedarray`). Pulse/Sentinel/Cortex/Edge/Prism/Reactor and LOCK-8/10/11 are no longer blocked on it — LOCK-11 itself is also done.
