...
| 65 | std/phonetic/ name-matching (nysiis cologne match_rating daitch_mokotoff phonex statcan_code roman_soundex onca lein phonetic_compare) — string algos, canonical reference-vector KATs. 12-skeptic adversarial audit recomputed canonical vectors INDEPENDENTLY (not trusting fleet KATs) + coverage-probed rule-heavy algos. Shipped **10**; **HELD caverphone + double_metaphone** — both are long-rule-table algos (50 / ~200 rules) and BOTH the fleet AND the auditor produced WRONG canonical values (I verified caverphone is canonically wrong on initial-vowel/internal-r/y names — drops initial vowel instead of →A, maps every r→A; DM missing initial-X→S + Spanish-LL, ~40/200 rules). No reference oracle available → deferred to attended effort w/ Apache-Commons/jellyfish oracle. FIXED before ship: phonex px_digit -1 dedup contract (was ignoring `prev`), nysiis ny_translate_char next-lookahead (EV/KN/PH/H/W now honored); cologne umlaut-comment + X-sentinel doc; daitch secondary-branch-dedup limitation documented. Rule-verified: Soundex R163/A261/T522, NYSIIS MACINTOSH→MCANTAS, Cologne 65752682, MRA BYRN, D-M 739400, LEIN digit-table, Jaro MARTHA/MARHTA 0.944 | 10 | *(this commit)* |
