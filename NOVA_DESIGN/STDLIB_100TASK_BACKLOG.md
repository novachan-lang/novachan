...
| 77 | std/cli/ command-line parsing (cli_flags options positional subcommand getopt longopt kvparse usage validate quotesplit envargs argparse) — pure string/list, NO side effects (env passed in), reference-vector KATs. Expands under-served cli (was 1 module). All 12 KATs green: flag clustering -vvv=3, --opt value/--opt=value, positional extraction w/ value-opts, subcommand, getopt cluster-expand, longopt =split + unambiguous-abbrev, k=v parse, usage formatting, validate, quote-aware split, env fallback, integrated argparse. Fleet flagged a compiler behavior (len() on a string returned from a fn reads 0; direct list-index return corrupts) worked around via single-exit `let v; return v` — matches [[return-null-mistag]] class, REPRO'd separately | 12 | *(this commit)* |
