...
| 52 | std/units/ conversions (length mass time temperature area volume speed pressure energy power data_size angle_unit) — SAFE domain (pure multiply, no trig). Audit recomputed ALL ~96 factors vs NIST/SI: all correct; 1 doc-only fix (cup label). NOTE: energy test = `_units_energy_test` (avoids `_energy_test` = std/physics/energy) | 12 | *(this commit)* |
