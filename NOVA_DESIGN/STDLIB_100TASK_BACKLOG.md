...
| 53 | std/money/ INTEGER money (minor currency format parse round allocate exchange tax discount aggregate accounting validate) — cents-as-int, no floats. Audit caught systematic NEG-numerator half_up bug (exchange/tax/discount) + format sign-position, FIXED + verified with neg probes. NOTE: 5 generic basenames collided (format/round/tax/validate/parse) → tests are `_money_<mod>_test`; 7 std tests | 12 | *(this commit)* |
