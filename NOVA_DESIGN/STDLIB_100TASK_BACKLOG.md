...
| 71 | std/itertools/ eager list transforms (it_chunk window take_drop flatten zip accumulate group unique interleave partition rotate product) — list/int only, NO float, NO HOF, reference-vector + round-trip KATs. Expands under-served itertools (was 1 module). All 12 KATs green: chunk/window/take/drop/slice/step, zip↔unzip round-trip, prefix sums/products/diffs, RLE encode↔decode round-trip, dedupe order-preserving, interleave/intersperse/roundrobin, split/halve/parity, rotate/cycle, cartesian product. Aliasing-safe (fresh inner list per iter — KATs assert distinct sublist values) | 12 | *(this commit)* |
