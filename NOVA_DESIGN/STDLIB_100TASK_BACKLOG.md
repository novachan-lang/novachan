...
| 51 | std/control/ control+robotics (pid statespace transferfn dcmotor so3 quatext trajectory motionprofile discretefilter ratelimit sim1d lqr) — audit: ratelimit REJECT (neg quantize) FIXED, pid/motionprofile guards added, so3/quatext self-audited. **robotkin HELD (13th): confirmed layout-dependent float-ABI codegen bug in rk_fk — cos on raw float param; needs ATTENDED compiler fix** | 12 | *(this commit)* |
