...
⛔ **Every figure in this §SCALE section was measured when `PrismConState` reached 109 leaves.** The tree is now **133** — typing 14 collection fields (§10.8) let `reach()` recurse into them. The numerators and percentages below are left at their as-measured values, because 89/109 = 82% and 51/109 = 47% are the numbers actually observed; re-basing them on 133 would pair old numerators with a new denominator and make the arithmetic wrong. §12's simulation is the one measured at 133.
