...
|---|---|---|---|
| **C** | raw scalar/float perf | float math **matches/beats C** (intrinsics, 1495bd3); scalar/int native; non-escaping structs stack-alloc'd (4a); **struct-math-via-fn-call 1.009× C (P1)**; tensor elementwise SIMD auto-vectorized; **tensor_matmul SIMD + AUTO-PARALLEL = BEATS C 1.32× (84e451a, 8 cores, zero user effort)** | construction-heavy loops ~3.8× (i64-handle ABI → Stage-5); SIMD on `[float]` **lists** (staged P2); matmul cache-blocking to amplify the parallel win past memory-bandwidth |
