...
|---|---|---|---|---|---|
| **MC.29** | ★ `app/prism_route.nova` — routing, deep-linked sheets, breadcrumbs, scroll restore | ✅ **DONE** | 2026-09-03 | 2026-09-03 | **78 assertions ALL PASS** (re-run by me). First module of `app/` (Tier 2), shaped as the reference for the nine that follow. ★ **Typed params are a MATCH CONSTRAINT, not a cast** — `:id:int` does not match `/users/abc` at all, so "abc" can never bind to something expecting a number. ★ **Ambiguity is REJECTED AT REGISTRATION**, never resolved by order: a first-wins router makes registration order load-bearing and invisible. Specificity is a total order, so `/users/new` and `/users/:id:int` coexist deterministically while two same-shape untyped routes are refused with WHY. ★ **Breadcrumbs are DERIVED** from segment-wise prefixes — no parent pointers to drift; proved with a title that deliberately does not match its slug. Deep-linked sheets reconstruct their background from the sheet's OWN params, so a cold load renders correctly underneath. Path segments are never percent-decoded (blocks `%2F` smuggling), and §15.1's route rule is re-validated INDEPENDENTLY here because browser history is a third untrusted-input boundary |
| **MC.30** | ★★ `app/prism_form.nova` — form state, autosave, draft recovery (#87) | ✅ **DONE** | 2026-09-04 | 2026-09-04 | **50 assertions, exit 0.** Arrived UNTESTED (its author was cut off before ever running the KAT) and crashed with an ACCESS VIOLATION; I bisected and fixed FOUR defects across two classes. ⛔ **Bools crossing a boundary** — a bool in a list cell and a bool PARAMETER both faulted; now int 0/1, matching `prism_event`/`prism_media`/`prism_notice`. ⛔ **A dangling `let x =`** with the value on the next line, ×3 — compiles inside an `if` block, `E0001` at function-body level, corrupts where it compiles. ★★ **AND THE REAL FIND:** merging seven constructor branches, each a DIFFERENT enum payload shape, into one `let fresh_r = if …` **CORRUPTS the struct** — it does not fail where built, it faults LATER when a field is read back. Minimal repro committed as `_branch_merge_test.nova`; fixed by EARLY RETURN per branch + a shared `_prism_fm_finish`. The KAT proves #87's two real properties: draft round-trip with a non-ASCII value, and a password field ABSENT from the recovered draft |
| **MC.26** | ★ `style/prism_print.nova` — print layout (#115) | ✅ **DONE** | 2026-09-03 | 2026-09-03 | **34 assertions ALL PASS.** Print is a DIFFERENT MEDIUM, not a narrow screen: it is PAGINATED, has NO interaction, and has no hover or scroll — so hidden content is **LOST**, not merely inconvenient. The web's answer (a stylesheet that hides things) is why printed pages lose content off the right edge. ⛔ **Units are integer MICROMETRES:** A4 = 210000×297000 µm and Letter = 215900×279400 µm are EXACT integers, which they are not as float millimetres (8.5in is fine, 215.9mm is not), and a float page height accumulates error across a hundred pages until the last one is short. ★ lines-per-page **FLOORS** — rounding up is how a page loses its last line off the bottom edge where nobody sees it. ★ **ZERO lines is ONE page**, because returning 0 makes a caller skip the header/footer pass for a document that still produces a sheet. ★ Margins that consume the page are **REJECTED, not clamped**: clamping yields a page with no printable area and every later division breaks |
| **MC.27** | ★ `widget/prism_notice.nova` — toast/snackbar queue (#71) | ✅ **DONE** | 2026-09-03 | 2026-09-03 | **34 assertions ALL PASS.** ⛔ **AN ERROR NEVER AUTO-DISMISSES**, nor does a warning — auto-dismissing an error is how a failure becomes invisible: the user looks away for four seconds and the only report that something went wrong is gone, unrecoverable. Enforced in the CONSTRUCTOR, not left to each caller's timeout argument, so it cannot be forgotten. ⛔ **Overflow evicts the oldest AUTO-DISMISSING notice, never the oldest outright.** An unbounded queue is a memory leak with a UI attached (a retry loop posts on every failure), but dropping the oldest unconditionally discards the error the user still needs while keeping a "Saved!" that does not matter. When EVERY slot requires acknowledgement, posting is **REFUSED** — the honest answer, since nothing droppable has been seen. Composed from the closed 22-primitive vocabulary (A7) |
| **MC.28** | ★ `widget/prism_media.nova` — video/audio playback (#82) | ✅ **DONE** | 2026-09-03 | 2026-09-03 | **40 assertions ALL PASS.** ⛔ **A VIDEO WITHOUT CAPTIONS IS UNREPRESENTABLE** through the ordinary constructor, and audio requires a transcript — the same move §10 makes with `by` on `each`/`grid`: the thing everyone forgets becomes a REQUIRED ARGUMENT. A "captions: optional" API is how every platform ended up with uncaptioned video; the default won every time. The escape hatch is `prism_md_video_no_captions` — named so it cannot hide, greppable in review, and it **DEMANDS A STATED REASON**, because an uncaptioned video with no explanation is indistinguishable from an oversight. ⛔ **AUTOPLAY REQUIRES MUTED**, enforced in the constructor — ★ **and UNMUTING CLEARS AUTOPLAY**, the half a naive implementation misses, since otherwise the forbidden pair is reachable backwards (mute → autoplay → unmute). Asserted both directions. The captions state is part of the RENDERED node, so an uncaptioned video renders `NO CAPTIONS` in any backend rather than being silently absent |
| **MC.25** | ★★ `render/prism_render_image.nova` — deterministic PNG — **`render/` 4/4; ALL SIX LAYERS COMPLETE** | ✅ **DONE** | 2026-09-03 | 2026-09-03 | **37 assertions ALL PASS, first run.** A real spec-conformant PNG writer: signature, IHDR, IDAT (zlib), IEND, correct CRC-32 per chunk and Adler-32 over the uncompressed stream. ★ **Checksums verified against PUBLISHED constants** — `crc32("IEND") == 0xAE426082` and `adler32("abc") == 0x024D0127` — because a self-consistently WRONG checksum yields a file every decoder rejects and no structural assertion would catch it. ★ **CRC-32/Adler-32 are implemented HERE, not borrowed:** the runtime's `crc32` builtin takes a STRING and derives its length from it, while a PNG chunk is BYTES that routinely contain 0x00 — exactly the truncation class fixed across ten functions this session. ★ **Deflate is "stored" (BTYPE=00), which IS a valid PNG:** there is no deflate in this runtime, and a file CLAIMING compression while carrying raw bytes would be invalid-but-sometimes-accepted, the worst outcome. Only `_img_zlib` changes when real deflate lands. Byte-level asserts cover the per-row FILTER BYTE (omitting it is the classic skew bug) and the LEN/NLEN pair a decoder validates. Text rasterisation is out of scope (needs a font engine — GPU backend, M2.x); `prism_img_skeleton` paints layout SHAPE instead, which is what a visual baseline wants: it moves when structure moves and not when copy changes |
...
| T15 | **M1.7 — THE BET-1 FALSIFIER** | ✅ EVIDENCE DELIVERED | 2026-09-04 | `NOVA_DESIGN/M1_7_FALSIFIER_RESULT.md` | **Bet 1 NOT falsified. M1.7's own >20-30% ratio criterion was INVALID** (on 5-leaf types a face reading ONE field scores 20%). Real measure: **median read-set = 1 leaf, mean 1.87**, slope flat (r²=0.15). Two failure modes located, **both measured tractable**: god-object state (100%) and delegation inheriting whole read-sets — the latter RECOVERED by constructor slicing (`prism_ss_is_usable` 11/11→**4/11**, slope 0.255→0.146), validated against a hand-derived answer. M3.4 needs leaf-granular tracking + field-to-field reconstructor flow; neither is research. Remaining unknown is SCALE (>100-leaf tree), not mechanism. Proxy analyser, NOT the compiler pass  ★ **SCALE ANSWERED 2026-09-05** on `prism_app_console` (133 leaves, depth 6, 26 faces, 94/94 KAT): **Bet 1 HOLDS.** Marginal read-set median **2 leaves** at 6× the state size; invalidation fan-out **7.5% of faces per change, max 12%**. ⛔ But my OWN replacement criterion ALSO failed (mean 8.2 > 6, max 47% > 25%) and is replaced again — read-set size is an INTERMEDIATE variable; fan-out is the outcome and is directly measurable. Degradation is entirely collection-driven ⇒ keyed sub-faces are now REQUIRED |
| T15b | **M3.4 reactivity — DESIGN + face-purity gate** | ✅ DESIGN DONE | 2026-09-04 | `NOVA_DESIGN/PRISM_M3_4_REACTIVITY_DESIGN.md` | Static read-sets are possible for PRISM and not for React/Solid/Svelte/Vue **because of the host language**, not because they missed it (dynamic property access, mutable closure scope, `eval`, `any`). Payoff: **zero runtime tracking cost** where Solid pays a getter per read. ⛔ **§4 collections are where this earns or loses** — a `list` is one leaf to the analysis but N rows at runtime; recommends **inferred keyed sub-faces**, rejects a static/runtime hybrid (two dependency systems = two failure modes). Failure hunt: 5 cases cost perf, **1 costs correctness silently** (a face reading module-level state) ⇒ must be a compile-time REJECTION. ✅ Detection half BUILT + gated (`tools/m34_face_purity.py`): **0 mutating / 0 computed initialisers across all 131 modules**, so the library is already compatible and enforcement needs no violation backlog cleared. Enforcement + the pass need GO  ★ **2026-09-05:** criterion reached its **4th and final form — invalidated WORK** (% of reachable → absolute size → fan-out over face DEFINITIONS → work); each earlier form failed for a recorded reason, the third because a definition count is blind to O(N) inside one face. Keyed sub-faces are **REQUIRED on COST**: without them a one-row edit is O(rows), **unbounded** (`selected_project` 51→1 leaves, `project_list` 32→0; 9/10 element types keyable). §9 **aggregates designed** (group/semigroup/holistic; count+sum are O(1) on delete, min/max rescan, holistic hopeless) — **build-or-not gated on measurement**, since `stat_row` alone would not justify incremental view maintenance. ⛔ Two holes found in my OWN design: the **changeset could not represent a row INSERT** (a correctness bug — leaf paths describe a field changing, not a collection's shape), and the HOF row resolved **from the compiler**: call-site specialisation is already the norm, but an **address-taken fn cannot be specialised at all** and reads fields BY NAME, so it needs a conservative fallback + diagnostic  ★ **BOTH GATING MEASUREMENTS DONE 2026-09-05.** §9.6 **aggregates: build Tier 1 ONLY** — 38/414 faces are aggregates, **82% group-class** (count/sum, O(1) on delete) and **ZERO holistic**, so Tier 3 was designed for a case that does not occur and Tier 2's 5 min/max are all root composers inheriting a callee's class. §10.7 **my Rule 1 premise was WRONG** — the behavioural rule I called "the rule that matters" **never fires alone** (0%); the naming heuristic I ranked weakest carries **90%**, and the two never disagree, so Rule 1's real job is **validator, not primary source**. ⛔ §10.8 **HARD PREREQUISITE FOUND**: key inference needs `list<T>`-typed collections, and **63 of 74 collection fields (85%) across 43 state types are bare `list`** — only `prism_app_console` is typed, so the whole corpus-wide number is one file. Nothing in §3/§4b/§9/§10 functions until those are typed. Mechanical, not RED-tier  ✅ **PREREQUISITE DONE 2026-09-05:** all **14 typeable collection fields typed `list<T>`** across 12 modules — 5 of them **already named the element type in a COMMENT** (`// list<GdIssue>`), so the change moved knowledge the authors had into the type where the compiler can read it. Visible collections **10→24**; key-inference coverage **83.3%**, and the 16.7% remainder is **not a gap** — 3 of 4 are derived report lists rebuilt wholesale (Rule 4 positional applies soundly), leaving **compound keys on exactly ONE collection** as the only real v2 item. ⛔ My §10.7 refutation of Rule 1 was itself an **artifact of the untyped corpus**: Rule 1 coverage rose 30%→47.6%→54.2% purely as collections became VISIBLE. ⛔ **BUT §10.12 (2026-09-05) found those Rule 1 figures are INFLATED** — the detector counted FILTER PREDICATES as lookups, and "Rule 1 wins on disagreement" would have keyed a notification queue by a **0/1 flag**. Policy withdrawn; re-measure pending. Gated on **all 20 transitively-affected KATs (RAN=20 FAILURES=0)** after the first attempt was a **FALSE GREEN** (CRLF work-list skipped every iteration while printing `failures: 0`)  ★ **2026-09-06: STEP 1 IMPLEMENTED, RED-tier gate PASSED.** `readset_of` is real compiler code (~406 lines), CI-gated. Reconverge byte-identical, both-mode regression 3592/0/0, KAT verified by direct observation. ⛔ Found (not yet fixed): the KAT covers only straight-line code, no `if`/`while`/`match` — must broaden before trusting this for real PRISM faces  ★ **2026-09-06: `@face` LANDED (`f6765c12`), M3.4 FOUNDATION COMPLETE.** Marks a face and enforces its contract at compile time — E1320 (needs a struct param), **E1321 (must not read mutable module state — the only rule here that costs CORRECTNESS: invisible to read-set analysis ⇒ face never re-runs ⇒ stale UI)**, E1322 (must not return its own state type = a reducer). No parser change needed — §17 found the general annotation mechanism already exists, collapsing §14's steps 3+4 into one change. All four behaviours probed by hand incl. the load-bearing negative: an UNANNOTATED fn violating all three still compiles, so this cannot break existing code. ⛔ **Nothing consumes readset_of yet** — next is §3 changeset + §4b keyed invalidation, the first non-reconverge-trivial change |
...
* **✅ `prism_ui_kit` CLOSED 2026-09-05** — `_kat_prism_ui_kit.nova`, **128 assertions**, all 11
  exported helpers. Its abort contract is now asserted directly (3/1/2/3/0 maker calls for
  all-succeed / fail-at-0 / fail-at-1 / fail-at-2 / empty), and `prism_ui_kv`'s abort *position*
