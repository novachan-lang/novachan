...
| **NEW / OPEN — `find_tag` reads OUT OF BOUNDS on any FOREIGN (FFI) pointer** | 0 | A | 🔴 **FOUND 2026-08-02 by an ASAN sweep; PRE-EXISTING and long-standing.** ASAN: `heap-buffer-overflow, READ of size 4` in `nova_mem_find_tag` (the rc check `((const int32_t*)ptr)[-2]`), reached via `channel_send` -> `deep_copy`. The pointer is a **sqlite3 database handle** — a 792-byte `malloc` from `openDatabase` — flowing through NOVA as a plain i64, because in the uniform-i64 model an FFI pointer IS just a value. The read lands 4 bytes BEFORE that foreign allocation. Reproduces in 3 DB tests (`forge_admin_test`, `forge_admin_model_test`, `forge_auth_sweep_test`); NOT caused by tonight's work (only a *call* to find_tag was added) and it predates the `IsBadReadPtr` removal in `b6debe3e` — that guard only proved READABILITY, and malloc metadata is readable, so it never caught this either. **Why the range guard misses it:** non-slab allocations use plain `calloc` and `nova_track_heap_bounds` widens a min/max ENVELOPE, so sqlite's mallocs (same process allocator) land inside `[heap_base, heap_top)`. In practice the 4 bytes are malloc metadata and always readable, so it does not crash today — but it is UB, it fails the project's ASAN-clean bar, and it can fault if a block starts a page. **It is ON THE CRITICAL PATH for Prism / Edge / Reactor, which pass foreign handles as values constantly.** **Neither cheap fix is sound:** a page-granular NOVA page-set still misses the case where a NOVA `calloc` block and a foreign block share a page. Exact membership needs EITHER (a) a segregated address space — all managed allocations sub-allocated from NOVA-owned chunks (extends the existing slab design), making the test an exact range check with NO header read and likely FASTER than today; or (b) a live-allocation registry — exact but costs an insert/erase per alloc/free plus a lookup on the hot path. **(a) is the recommendation, and it is a real design fork with real spend — owner's call before starting.** | |
