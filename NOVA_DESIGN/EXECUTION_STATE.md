...
| N>1 per-carrier I/O | 0-C | B | 🔄 **MEASURED 2026-08-02 — the "goal met" claim holds for CORRECTNESS but NOT for SCALING.** Correctness: an 8-worker spawn+channel workload gives an IDENTICAL total at N=1/2/4/8 and under FULLRC (deterministic, no races observed). Scaling on 8 logical cores, 3 runs (`_bench_mn_scaling`), embarrassingly-parallel and share-nothing: N=1 53.0/57.0/55.2 ms → N=2 29.8/31.0/28.6 (~1.9x) → N=4 **31.7/19.8/27.5 (1.7x-2.8x, a 60% SPREAD)** → N=8 26.5/18.4/19.2 (~3.0x). That is ~37% efficiency at 8 cores, and N=4 straddles the plan's own **>1.8x @ 4-worker perf gate** depending on the run. The instability at N=4 points at run-queue/scheduler contention — consistent with the literal per-carrier shard being absent. **So this row stays OPEN on the scaling goal**; closing it as "goal met" would have been wrong. Single-poller (S-a/S-b/S-c) delivered correctness, not throughput. | |
