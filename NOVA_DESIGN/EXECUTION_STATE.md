...
| **NEW / OPEN — use-after-free on a dict-stored string across HTTP requests** | 0 | A | 🔴 **PRE-EXISTING UAF, found 2026-08-02 by running the full CI.** `real_http_api` stores `{"name": <string>}` in a list of dicts. CREATE and LIST render `"widget"` correctly, but a later `GET /api/items/1` renders the name as a large integer that CHANGES every run (1985111514056 / 2305354622648 / 2420455426424) — those are heap POINTER values, i.e. the string's tag lookup fails and `str()` falls back to rendering the raw handle as an int. The string was freed while still reachable from the dict. **CONTROLLED: reproduces identically when linked against HEAD's runtime, so it is NOT from tonight's work.** Same failure class as the int/pointer ambiguity and the struct-field leak. Sequencing narrows it: the name renders correctly in the CREATE response AND in the LIST response, then is dead by the by-id GET — so it dies during/after serving the list. **THREE plausible causes were tested and REFUTED** (probes `_uaf_probe3..6`, 0/30 corrupted each): (1) dict LITERAL construction failing to retain its values; (2) `push()` into a list received as a PARAMETER, with the value local to the callee; (3) `for x in xs` releasing borrowed elements, run twice. All three are SOUND. **LOCALIZED by instrumenting `_handle` to print the stored names on entry to EVERY request:** at the LIST request the names are still `[widget|gadget]`; at the very next request BOTH are already pointers. So they die **during the list handler** (`_items_json` -> `_item_json`), not at connection teardown and not on the by-id path. **FIVE mechanisms tested and REFUTED, 0/30 corrupted each:** (1) dict LITERAL retain; (2) `push()` into a list passed as a PARAMETER with a callee-local value; (3) `for x in xs` releasing borrowed elements (run twice); (4) a spawned GREEN TASK owning the list while names are built by `trim(slice(...))` from channel-received data; (5) passing each dict to a HELPER FN that reads its fields (the exact `_item_json` shape). All sound. The ONLY variable left is that the real strings come from **`tcp_recv`** (`nova_heap_alloc(n+1, NOVA_MEM_RAW)`). Where to look: `nova_heap_alloc` bump-allocates from a **per-task transparent arena** (`nova_cur()->active_arena`) when one is active, and several paths (channel send, spawn, mailbox) deliberately NULL the arena first so a value that ESCAPES is materialized on the RC heap instead — a missing guard of that kind fits the evidence exactly. Note `nova_arena_mode` makes `nova_rc_inc`/`nova_rc_dec` NO-OPS, so verify whether it is on in this path before assuming refcounts apply. *(Method note: an earlier probe appeared to reproduce 40/40 but was MY bug — an unescaped `{` in a NOVA string, which interpolates, so the slice extracted the wrong substring. Escape braces as `\{` in probes, and re-derive a repro before trusting it.)* | |
