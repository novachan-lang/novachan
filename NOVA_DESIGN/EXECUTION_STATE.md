...
| module-level `let X=<init>` DROPS the initializer (all globals zero-init) | 0-A | B(XL) | ⬜ DIAGNOSED, XL — module lets are NOT real globals: each fn that references one gets its OWN zero'd `%slot` copy, so only `nova_main`'s scope sees the init. Backends INCONSISTENT: cg/emit resolves via `collect_locals` (assign/let targets only → read-only refs get no alloca), ire_line injects zero'd copies. FIX = true LLVM globals `@nova_g_X` (cg: get_slot@4078 global-aware after current_slots check → shadow-safe; skip alloca for globals in emit_nova_main@7044; emit decls; nova_main stores init → global) + the analogous ire_line change; must reconverge byte-identical. Not a quick fix — dedicated. Workaround: init inside a fn (natural). | |
