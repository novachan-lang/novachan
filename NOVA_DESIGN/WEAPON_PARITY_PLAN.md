...
| 5.7 | ARM/AArch64 native | Go/Rust/all | L | **✅ DONE 2026-08-28 — CI-GATED ON BOTH AArch64 TARGETS.** "needs hardware" was simply wrong twice over: `macos-latest` is arm64 and `ubuntu-24.04-arm` is free for public repos. **`aarch64-apple-darwin`** (Mach-O + libSystem) and **`aarch64-unknown-linux-gnu`** (ELF + glibc) BOTH build, self-identify their native triple, and **reconverge gen5 == gen6 byte-identical (canonical 3-pass)**. Linux-ARM64 passed on its FIRST run with zero porting — a proactive sweep had already confirmed no x86 intrinsics and correctly arch-guarded asm, so the AAPCS64 fiber-switch was the only arch-specific code and it was already right. Seed: `nova_compiler_linux_arm64.ll` |
