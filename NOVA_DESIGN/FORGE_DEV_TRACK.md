...
| 23 | gRPC unary dispatch (service map, fn-value handlers, envelopes) | forge_grpc.nova | _fdb_one forge_grpc_test (to write) | register "/echo.Echo/Say"→echo handler; dispatch(framed[1,2,3])→{framed:[0,0,0,0,3,1,2,3],status:0}; unknown path→status 12; response_ok/error envelopes. **SECURITY TODO (test pass MUST fix): grpc_deframe has NO bounds check — malformed/short body = OOB read/DoS; add len(buf)>=off+5+length guard before exposing to untrusted clients** | untested (syntax✓) |
