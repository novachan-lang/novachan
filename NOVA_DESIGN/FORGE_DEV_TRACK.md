...
| 15 | CSV export RFC 4180 (HIGH-LEVEL NOVA: comprehensions+join) | forge.nova | grep:csv_of | csv_row([a,b])→"a,b"; field with comma/quote/CRLF gets quoted+quote-doubled; csv_of CRLF-joins rows; resp_csv→200 text/csv attachment | untested (syntax✓) |
| 16 | NDJSON streaming/bulk (HIGH-LEVEL NOVA: comprehension+join) | forge.nova | grep:ndjson_of | ndjson_of([{a:1},{a:2}])→two JSON lines LF-joined; resp_ndjson→200 application/x-ndjson | untested (syntax✓) |
| 17 | Proactive media-type negotiation RFC 7231 (HIGH-LEVEL: comprehension, permille q) | forge.nova | grep:negotiate_type | Accept:application/json;q=0.9,text/html → offered[json,html] picks html (q=1.0>0.9); q=0 excludes; most-specific range wins its q; no match→"" (caller 406); absent Accept→offered[0] | untested (syntax✓) |
| 18 | JSON-RPC 2.0 codec (request/notification/success/error/batch/parse+accessors) | forge_jsonrpc.nova | _fdb_one forge_jsonrpc_test (to write) | jsonrpc_request("sum",[1,2],1)→{jsonrpc:2.0,method,params,id}; error nests code+message; is_notification=no id; parse round-trips; batch=JSON array | untested (syntax✓) |
...
| 20 | gRPC wire framing + path routing + status/trailers (leaf, protobuf-agnostic) | forge_grpc.nova | _fdb_one forge_grpc_test (to write) | grpc_frame([1,2,3])→[0,0,0,0,3,1,2,3]; deframe round-trips→[[1,2,3],8,0]; path_service("/echo.Echo/Say")→"echo.Echo", method→"Say"; trailers dict; 10 status codes | untested (syntax✓) |
| 21 | Transactional outbox (ACTOR, at-least-once, poll-then-ack) | forge_outbox.nova | _fdb_one forge_outbox_test (to write) | add 2→ids 1,2 pending=2; poll(10) both; ack(1)→pending=1; poll returns only id2; poll does NOT mark delivered (crash re-delivers) | untested (syntax✓) |
| 22 | W3C Trace Context propagation (traceparent parse/child/continue, CSPRNG ids) | forge.nova | grep:trace_continue | trace_start(true)→00-<32hex>-<16hex>-01; parse validates hex+non-zero+sampled bit; trace_child keeps trace_id+flags new span_id; malformed→fresh; trace_id_of for log correlation | untested (syntax✓) |
| 23 | gRPC unary dispatch (service map, fn-value handlers, envelopes) | forge_grpc.nova | _fdb_one forge_grpc_test (to write) | register "/echo.Echo/Say"→echo handler; dispatch(framed[1,2,3])→{framed:[0,0,0,0,3,1,2,3],status:0}; unknown path→status 12; response_ok/error envelopes. grpc_deframe now BOUNDS-CHECKED (bee5245-follow): short/lying frame → malformed sentinel [[],off,-1]; grpc_dispatch returns INVALID_ARGUMENT (no OOB read). SECURITY GAP CLOSED | untested (syntax✓) |
...
| 25 | Circuit breaker (ACTOR, CLOSED/OPEN/HALF_OPEN, monotonic) | forge_circuit.nova | _fdb_one forge_circuit_test (to write) | threshold=3: 3 failures→open; allow→0 while open; after reset_ms allow→1 (half-open probe); probe success→closed, probe fail→open; circuit_state does NOT advance timer (only allow does) | untested (syntax✓) |
| 26 | Retry exponential backoff (PURE, cap, caller-jitter, schedule) | forge_retry.nova | _fdb_one forge_retry_test (to write) | base=100 max=2000: delays 100,200,400,800,1600,2000-cap; retry_should(2,3)=T (3,3)=F; apply_jitter(800,500)=400; schedule(4,100,2000)=[100,200,400,800]; total_ms=1500. sum not a builtin (fold) | untested (syntax✓) |
...
| 28 | GraphQL variables ($x) — production-usable | forge_graphql.nova | _fdb_one forge_graphql_test | tokenizer emits var token for $name; arg $id → {__var:id} marker; _gql_apply_vars substitutes from variables dict before resolve (missing→""); gql_execute_vars / gql_execute_limited_vars; execute/limited delegate with {} | untested (syntax✓) |
