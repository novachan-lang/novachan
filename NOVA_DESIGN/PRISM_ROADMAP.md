...
| ID | Milestone | Deliverable | **Exit criterion** | Radius | Wks |
|---|---|---|---|---|---|
| **M3.1** | `view` syntax + vocabulary | Lexer/parser for `view`, indented children, `-> expr` handlers, **indented handler blocks**; the 26 primitives typed | The counter syntax from the plan compiles and runs; parser round-trip tests; a malformed view gives a **located, actionable** error; **FULL ARC** | **RED** | 4-6 |
