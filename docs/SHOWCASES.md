...
| [`showcase_shapes_test.nova`](../nova-compiler/test_programs/showcase_shapes_test.nova) | **Type-system stress: all features at once** — enum/ADT with payloads + exhaustive `match` + traits & method dispatch + generics with a trait bound (`fn<T: Describable>`) + the built-in `Result` (ok/err) + enum values in a list; Heron's `sqrt` path | ✅ passes clean — **enum/match/trait/generic/Result compose in one real program**; areas exact (incl. `sqrt`) |
